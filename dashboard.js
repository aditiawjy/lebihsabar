(() => {
	...
	var BACKTEST_WEAK_MIN_SAMPLE = 5;
	var BACKTEST_WEAK_DROP_PCT = 15;
	var RELIABLE_MIN_SAMPLE = 5;
	var RELIABLE_MIN_WILSON = 70;

	// Wilson score lower bound (95%) -- mirror dari dashboard_cache.php
	function wilsonLowerBound(h, n) {
		if (!n || n <= 0) return 0;
		var z = 1.96,
			p = h / n;
		var denom = 1 + (z * z) / n;
	...
	// Base rate: persen match (yang ada gol 1H) yang berlanjut gol di babak 2, tanpa pattern.
	function computeBaseRate() {
		var all =
			INITIAL_DATA && Array.isArray(INITIAL_DATA.all_matches)
		...
		return n > 0 ? (h / n) * 100 : 0;
	}

	// Penilaian keandalan -- mirror buildReliability() di PHP.
	function buildReliabilityHtml(recent, allTimePct, baseRate) {
		var t = (recent && recent.t) || 0;
		var h = (recent && recent.h) || 0;
		var pct = recent ? recent.pct : null;
		if (pct === null || pct === undefined || t < RELIABLE_MIN_SAMPLE) {
			return (
				'<span style="color:#8b949e" title="Sampel recent < ' +
				RELIABLE_MIN_SAMPLE +
				', belum bisa dinilai">Sampel kecil</span>'
			);
		}
		var wlo = wilsonLowerBound(h, t);
		var drop = allTimePct - pct;
		var label, color, tip;
		if (pct < baseRate || drop >= 20) {
			...
			tip =
				...
				"% atau turun " +
				drop +
				"pt dari all-time -- tidak lebih baik dari tebak buta";
		} else if (wlo < RELIABLE_MIN_WILSON) {
			label = "Hati-hati";
			color = "#d29922";
			tip =
				"Wilson95 bawah " +
				...
				"% < " +
				RELIABLE_MIN_WILSON +
				"% -- sampel belum cukup untuk dipercaya";
		} else {
			label = "Andal";
			color = "#3fb950";
			tip =
	...
	function isSettledSummaryResultValid(result) {
		...
		if (historical) {
			...
			// Jika shape ini sudah tidak lagi cocok dengan definisi pattern terkini
			// (mis. di-exclude saat menaikkan akurasi), hasil settled-nya basi -> buang
			// agar tally W/L selaras dengan pattern, bukan menampilkan kekalahan usang.
			if (
				result.state &&
				Array.isArray(result.state.h1s) &&
				!matchesSummaryPatternLive(result.pid, result.state)
			) {
				return false;
			}
			return true;
		...
		try {
			...
			settledSummaryResults = Array.isArray(parsed.results)
				? parsed.results.filter(
						(result) => result && isActivePatternId(result.pid),
					)
				: [];
			pruneSettledSummaryResults();
		} catch (e) {
	...
	function restoreSettledLateState() {
		try {
			...
			settledLateResults = Array.isArray(parsed.results)
				? parsed.results.filter(
						(result) => result && isActivePatternId(result.pid),
					)
				: [];
	...
	function updateBacktestAlert(patterns) {
		var box = document.getElementById("backtest-alert");
		if (!box) return;
		var weak = patterns.filter(
			(p) =>
				p.recent &&
				p.recent.pct !== null &&
				p.recent.pct !== undefined &&
				(p.recent.t || 0) >= BACKTEST_WEAK_MIN_SAMPLE &&
				p.recent.pct <= (p.pct || 0) - BACKTEST_WEAK_DROP_PCT,
		);
		if (weak.length === 0) {
			box.style.display = "none";
			return;
		}
		var listEl = document.getElementById("backtest-alert-list");
		if (listEl) {
			listEl.textContent = weak
				.map(
					(p) =>
						p.id +
						" (" +
	...
	function renderSummaryTable(patterns) {
		...
		var summaryBaseRate = computeBaseRate();
		...
		if (sorted.length === 0) {
			tbody.innerHTML =
				...
				"Belum ada pattern dengan sampel cukup (min " +
				SUMMARY_MIN_SAMPLE +
				" match per pattern). " +
				patterns.length +
				" pattern sedang mengumpulkan data.</td></tr>";
			return;
		}
		tbody.innerHTML = sorted
			.map(
				(p) =>
					...
					"</td>" +
					'<td class="recent-cell" style="font-size:0.8rem;white-space:nowrap;">' +
					(p.recent && p.recent.html
						? p.recent.html
						: '<span style="color:#484f58">\u2014</span>') +
					"</td>" +
					'<td class="reliability-cell" style="font-size:0.8rem;white-space:nowrap;">' +
					buildReliabilityHtml(p.recent, p.pct || 0, summaryBaseRate) +
					"</td>" +
	...
	function renderLateTable(latePatterns) {
		function isVisibleLatePattern(lp) {
			...
			return total >= LATE_MIN_SAMPLE || (total >= 5 && hits === total);
		}
	...
	function matchesP12Summary(s) {
		...
		// Exclusion struktural: 20min AHAH 2-2 mnt 3-9 tidak hasilkan gol 2H
		if (
			s.league === "20min" &&
			seq === "AHAH" &&
			s.sc_h === 2 &&
		...
		return (
			...
			(s.league === "15min" &&
				...
				last <= 7 &&
				s.max_gap <= 4 &&
				(s.min_gap === 0 || s.max_gap >= 3 || last === 7)) ||
			(s.league === "15min" &&
				seq === "HHAH" &&
				(s.max_gap >= 3 || first === 0 || first === 2)) ||
			(s.league === "15min" &&
				seq === "AAHH" &&
				first <= 1 &&
				s.min_gap <= 1 &&
				s.max_gap >= 3) ||
			(s.league === "15min" && seq === "HHAA" && last <= 7) ||
			(s.league === "16min" &&
				seq === "AAHA" &&
				s.min_gap >= 1 &&
				s.max_gap <= 2) ||
	...
	var P77_SINGLE_GOAL_SIGNATURES = Object.freeze({
		"16min|1|A|0-1|3|21": true,
		"16min|1|A|0-1|2|14": true,
		"16min|1|A|0-1|5|21": true,
		"16min|1|H|1-0|4|20": true,
		"16min|1|H|1-0|6|18": true,
		"16min|1|H|1-0|1|14": true,
		"16min|1|A|0-1|4|21": true,
		"16min|1|H|1-0|3|15": true,
		"16min|1|A|0-1|4|11": true,
		"16min|1|H|1-0|2|21": true,
		"16min|1|H|1-0|4|21": true,
		"16min|1|H|1-0|8|15": true,
	});
	...
	function isP83StructuralMissSummary(s) {
		...
		// 16min HHA 2-1 mnt 0-4 jam 16 (shape sama dgn 1 hit jam 22)
		return (
			s.league === "16min" &&
			arrayEqualsJS(seq, ["H", "H", "A"]) &&
			s.sc_h === 2 &&
			s.sc_a === 1 &&
			s.h1_first === 0 &&
			s.h1_last === 4 &&
	...
	function matchesP79Summary(s) {
		if (!s) return false;
		var seq = Array.isArray(s.h1s) ? s.h1s : [];
		// Pengecualian struktural (tanpa nama tim): 16min HA 1-1 first1 last6 maxgap5 -> tanpa gol 2H
		if (
			s.league === "16min" &&
			seq.join("") === "HA" &&
			s.sc_h === 1 &&
		...
		return (
			...
			s.h1_first <= 1 &&
			s.max_gap >= 5 &&
			!(s.h1_first === 1 && s.h1c === 2 && s.max_gap >= 7)
		);
	}
	...
	function matchesP83Summary(s) {
		...
		if (s.league === "16min" && seq.join("") === "HA" && s.sc_h === 1 && s.sc_a === 1 && s.h1_first === 1 && s.h1_last === 6 && s.max_gap === 5) return false;
		// Pengecualian struktural (tanpa nama tim): shape yang tidak menghasilkan gol 2H
		if (
			(s.league === "20min" && seq.join("") === "AH" && s.h1_first === 5 && s.h1_last === 8) ||
			(s.league === "16min" && seq.join("") === "AH" && s.h1_first === 6 && s.h1_last === 7) ||
			(s.league === "20min" && seq.join("") === "HH" && s.sc_h === 2 && s.sc_a === 0 && s.h1_first === 3 && s.h1_last === 9) ||
			(s.league === "16min" && seq.join("") === "HAH" && s.sc_h === 2 && s.sc_a === 1 && s.h1_first === 1 && s.h1_last === 4) ||
			(s.league === "20min" && seq.join("") === "HAHA" && s.sc_h === 2 && s.sc_a === 2 && s.h1_first === 5 && s.h1_last === 8) ||
			(s.league === "20min" && seq.join("") === "AA" && s.sc_h === 0 && s.sc_a === 2 && s.h1_first === 3 && s.h1_last === 9) ||
			(s.league === "20min" && seq.join("") === "HAHA" && s.sc_h === 2 && s.sc_a === 2 && s.h1_first === 0 && s.h1_last === 6 && s.min_gap === 0 && s.max_gap === 3) ||
			(s.league === "15min" && seq.join("") === "HAA" && s.sc_h === 1 && s.sc_a === 2 && s.h1_first === 0 && s.h1_last === 7 && s.min_gap === 2 && s.max_gap === 5) ||
			(s.league === "15min" && seq.join("") === "AAA" && s.sc_h === 0 && s.sc_a === 3 && s.h1_first === 2 && s.h1_last === 7 && s.max_gap === 4 && s.kickoff_hour === 16 && s.kickoff_minute === 31)
		) {
			return false;
		}
		return (
	...
	function isP86StructuralMissSummary(s) {
		if (!s) return false;
		return (
			(s.league === "20min" &&
	...
	function matchesP86Summary(s) {
		if (!s || s.h1c <= 0 || isP86StructuralMissSummary(s)) return false;
		var diff = s.sc_h - s.sc_a;
		var lastScorer =
			Array.isArray(s.h1s) && s.h1s.length ? s.h1s[s.h1s.length - 1] : "";
		return (
			(s.max_gap === 4 && diff >= 1 && s.switches === 1) ||
			(s.max_gap === 4 && s.min_gap === 1 && diff >= 1) ||
			(s.max_gap === 4 && diff >= 1 && lastScorer === "A") ||
			(s.max_gap === 4 && s.min_gap === 2 && Math.abs(diff) <= 1) ||
			(s.max_gap === 4 && s.min_gap === 2 && s.switches >= 1) ||
			(s.max_gap === 4 && s.min_gap === 2 && s.max_run <= 2) ||
			(s.league === "16min" && s.h1_last === 5 && s.min_gap <= 1)
		);
	}
		...
		switch (pid) {
			...
			case "P6":
				return (
					...
					span >= 5 &&
					s.h1_first !== 1 &&
					!(kickoffHour === 14 && kickoffMinute === 1) &&
					!(kickoffHour === 22 && kickoffMinute === 46) &&
					!(kickoffHour === 23 && kickoffMinute === 2) &&
					!(kickoffHour === 20 && kickoffMinute === 31)
				);
			case "P7":
				return (
					s.h1c === 2 &&
			...
			case "P58":
				// Exclusion struktural (tanpa tim): 20min HH 2-0 3-9 / AA 0-2 3-9 tanpa gol 2H
				if (
					s.league === "20min" &&
					s.h1_first === 3 &&
					s.h1_last === 9 &&
					((arrayEqualsJS(s.h1s, ["H", "H"]) && s.sc_h === 2 && s.sc_a === 0) ||
						(arrayEqualsJS(s.h1s, ["A", "A"]) && s.sc_h === 0 && s.sc_a === 2))
				) {
					return false;
				}
				if (s.league === "16min" && arrayEqualsJS(s.h1s, ["H", "A"]) && s.sc_h === 1 && s.sc_a === 1 && s.h1_first === 1 && s.h1_last === 6 && s.max_gap === 5) {
					return false;
				}
				// 15min AAA 0-3 mnt 2-7 ko 16:31 tanpa gol 2H
				if (
					s.league === "15min" &&
					arrayEqualsJS(s.h1s, ["A", "A", "A"]) &&
					s.sc_h === 0 &&
			...
			case "P64":
				// Exclusion struktural (tanpa tim): 15min HAAA 1-3 mnt 0-5 tanpa gol 2H
				if (
					s.league === "15min" &&
					arrayEqualsJS(s.h1s, ["H", "A", "A", "A"]) &&
					s.sc_h === 1 &&
	...
	function notifyLiveSignals(signalMatches) {
		if (!LIVE_SIGNALS_ENABLED || !signalMatches || !signalMatches.length) return;
		var TTL = 2 * 3600 * 1000, now = Date.now(), sent = {};
		try { sent = JSON.parse(localStorage.getItem("liveTgSent") || "{}"); } catch (e) { sent = {}; }
		Object.keys(sent).forEach(function (k) { if (now - sent[k] > TTL) delete sent[k]; });
		signalMatches.forEach(function (sm) {
			var ids = sm.signals.map(function (s) { return s.id; }).sort().join(",");
			var key = sm.home + " vs " + sm.away + "|" + ids;
			if (sent[key]) return;
			sent[key] = now;
			var text = "Live Signal Pattern: " + sm.home + " vs " + sm.away + " | " + sm.league + " | Skor " + sm.score + " | " + sm.status + " | Pattern: " + ids;
			console.log("[TG] kirim signal:", key);
			fetch("tg_send.php", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: text, key: key }) })
				.then(function (r) { return r.json(); })
				.then(function (j) { console.log("[TG] hasil:", key, j); })
				.catch(function (e) { console.log("[TG] gagal:", key, e); });
		});
		try { localStorage.setItem("liveTgSent", JSON.stringify(sent)); } catch (e) {}
	}
	...
	function renderLiveCards(matches, livePayload) {
		...
		notifyLiveSignals(signalMatches);
		renderLiveAlerts(signalMatches);
	}

	// Parse odds string "FT. O/U: O0.75:2.10 | O1.0:1.80" untuk selection target.
	// Market FT dan 1H dipisah agar tidak tercampur (replika getOverOddsBySelection).
	function parseLiveOverOdds(m, marketFilter) {
		var wanted = ["o0.75", "o1.0", "o1.25"];
		var found = {};
		var oddsLines = Array.isArray(m && m.odds) ? m.odds : [];
		for (var i = 0; i < oddsLines.length; i++) {
			var parts = String(oddsLines[i] || "").split(": ");
			if (parts.length < 2) continue;
			var marketName = parts[0].trim().toLowerCase().replace(/\s+/g, "");
			if (marketName.indexOf(marketFilter) === -1) continue;
			var options = parts.slice(1).join(": ").split(" | ");
			for (var j = 0; j < options.length; j++) {
				var sep = options[j].indexOf(":");
				if (sep === -1) continue;
				var label = options[j].slice(0, sep).trim().toLowerCase();
				var val = parseFloat(options[j].slice(sep + 1).trim().replace(",", "."));
				if (!isFinite(val)) continue;
				for (var w = 0; w < wanted.length; w++) {
					if (label === wanted[w] && found[wanted[w]] === undefined) {
						found[wanted[w]] = val;
					}
				}
			}
	...
	function buildLiveOddsHtml(m) {
		var pieces = [];
		[["FT", "ft.o/u"], ["1H", "1h.o/u"]].forEach(function (mk) {
			var ou = parseLiveOverOdds(m, mk[1]);
			var ouParts = [];
			["o0.75", "o1.0", "o1.25"].forEach(function (sel) {
				if (ou[sel] !== undefined) ouParts.push(sel + "=" + ou[sel].toFixed(2));
			});
			if (ouParts.length) pieces.push(mk[0] + ": " + ouParts.join(" · "));
		});

		var ng = m && m.nextGoalOdds;
		if (ng && typeof ng === "object") {
			var ngParts = [];
			if (ng.home) ngParts.push("H " + ng.home);
			if (ng.away) ngParts.push("A " + ng.away);
			if (ng.none) ngParts.push("N " + ng.none);
			if (ngParts.length) pieces.push("NG: " + ngParts.join(" · "));
		}

		if (!pieces.length) return "";
		return (
			'<div class="live-odds" style="margin-top:6px;font-size:0.72rem;color:var(--text-muted);font-family:monospace;">' +
			escHtml(pieces.join("  |  ")) +
			"</div>"
		);
	}

...
// P3: AH seri 1-1, gap >= 3, lm>=4, 15/16min (95%)
$p3 = array_filter($matches, fn($m) => in_array($m['league'],['15min','16min']) && $m['h1c']==2 && $m['sc_h']==1 && $m['sc_a']==1 && $m['h1s']==['A','H'] && ($m['h1'][1]['min']-$m['h1'][0]['min']) >= 3 && $m['h1_last']>=4);
...
// P18: Span 1H >= 6 mnt, gol 1H >= 3, selisih HT <= 2, sw>=2, min_gap>=1
$p18 = array_filter($matches, fn($m) => $m['h1c']>=3 && ($m['h1_last']-$m['h1_first'])>=6 && abs($m['sc_h']-$m['sc_a'])<=2 && $m['max_run']<=2 && $m['switches']>=2 && $m['min_gap']>=1);
...
// P32: Span >=8 mnt + gol 1H >=2 + selisih<=3, 20min
$p32 = array_filter($matches, fn($m) => $m['league']==='20min' && $m['h1c']>=2 && ($m['h1_last']-$m['h1_first'])>=8 && abs($m['sc_h']-$m['sc_a'])<=3 && $m['min_gap']>=1);
...
// P37: First AWAY + last AWAY + span >= 6 mnt + (n1h>=3 or lm<=7) + min_gap>=1
$p37 = array_filter($matches, fn($m) => $m['h1c']>=2 && count($m['h1s'])>0 && $m['h1s'][0]==='A' && $m['h1s'][count($m['h1s'])-1]==='A' && ($m['h1_last']-$m['h1_first'])>=6 && ($m['h1c']>=3 || $m['h1_last']<=7) && $m['min_gap']>=1);
// P39: Gol 1H >= 3 + span >= 7 mnt + selisih<=3, 20min
$p39 = array_filter($matches, fn($m) => $m['league']==='20min' && $m['h1c']>=3 && ($m['h1_last']-$m['h1_first'])>=7 && abs($m['sc_h']-$m['sc_a'])<=3 && $m['min_gap']>=1);
...
$patterns = [
    ...
    ['id'=>'P3','label'=>'AH gap >=3 mnt + lm>=4, 15min/16min','data'=>$p3],
    ...
    ['id'=>'P18','label'=>'Span 1H >= 6 mnt + gol 1H >= 3 + selisih <=2 + sw>=2 + min_gap>=1','data'=>$p18],
    ...
    ['id'=>'P32','label'=>'Span 1H >=8 mnt + gol >=2 + selisih <=3, 20min','data'=>$p32],
    ...
    ['id'=>'P37','label'=>'First & last scorer AWAY + span >=6 mnt (n1h>=3 atau lm<=7) + min_gap>=1','data'=>$p37],
    ['id'=>'P39','label'=>'Gol 1H >=3 + span >=7 mnt + selisih <=3, 20min','data'=>$p39],
];

...
$response = [
    ...
    'patterns' => buildPatternSummary($data['patterns'], $oldSnapData, $oldSnapTime, $currentSnapTime, dashboardLatestMatchTs($data['all_matches'] ?? [])),
    'next_patterns' => buildNextPatternSummary($data['next_patterns'], $oldSnapData, $oldSnapTime, $currentSnapTime),
    'late_patterns' => buildLatePatternSummary($data['late_patterns'] ?? [], $oldSnapData, $oldSnapTime, $currentSnapTime, dashboardLatestMatchTs($data['all_matches'] ?? [])),
...
function buildPatternSummary(array $patterns, array $oldSnapData, ?int $rangeStart, ?int $rangeEnd, ?int $latestTs = null): array {
    ...
    return array_map(function($p) use ($oldSnapData, $rangeStart, $rangeEnd, $latestTs) {
        $total = count($p['data']);
        ...
        $recent = buildRecentStats($p['data'], fn($m) => $m['h2c'] > 0, $latestTs, (int)$pct);
        return [
            'recent' => $recent,
            'id' => $p['id'],
            'label' => $p['label'],
...
function buildLatePatternSummary(array $latePatterns, array $oldSnapData, ?int $rangeStart, ?int $rangeEnd, ?int $latestTs = null): array {
    ...
    return array_map(function($lp) use ($oldSnapData, $rangeStart, $rangeEnd, $latestTs) {
        $total = count($lp['data']);
        $lateTarget = $lp['target'] ?? 'has_late';
        $lateHits = count(array_filter($lp['data'], fn($m) => $m[$lateTarget] ?? false));
        ...
        $recent = buildRecentStats($lp['data'], fn($m) => $m[$lateTarget] ?? false, $latestTs, (int)$pct);
        return [
            'recent' => $recent,
            'id' => $lp['id'],
            'label' => $lp['label'],

...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P17', 'label'=>'First 1H mnt 1-2 + last mnt 7 + min_gap>=2 + switches>=1 + first scorer AWAY + (20min atau 15min first=1), kecuali Bordeaux vs Lyon', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1c']>=2 && $m['h1_first']>=1 && $m['h1_first']<=2 && $m['h1_last']==7 && $m['max_gap']>=2 && $m['min_gap']>=2 && $m['switches']>=1 && count($m['h1s'])>0 && $m['h1s'][0]==='A' && ($m['league']==='20min' || ($m['league']==='15min' && $m['h1_first']===1)) && !(trim($m['home'])==='Girondins de Bordeaux (V)' && trim($m['away'])==='Olympique Lyonnais (V)')))],

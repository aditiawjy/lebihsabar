...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P35', 'label'=>'AWAY shortlist: Mexico / Belgium / Germany / AS Monaco / Wales / Portugal / Osasuna / Austria / Poland / Croatia / Algeria + gol 1H>=2 + first>=3 + max_run<=2 + last gol 1H>=5 + min_gap>=2 + bukan HH mnt 3&5 + bukan HA 1-1 mnt 3-5 + bukan HAA mnt 3-7, atau 16min shortlist first=1 + h1c>=3, atau shortlist first=0 + h1c>=3 + last>=6 + last scorer AWAY, atau Portugal single AWAY, atau h1c=3 scorer HHA min_gap=1', 'data'=>array_values(array_filter($matches, fn($m) => (in_array(trim($m['away']), $p35_teams) && $m['h1c']>=2 && $m['h1_first']>=3 && $m['max_run']<=2 && $m['h1_last']>=5 && $m['min_gap']>=2 && !(count($m['h1s'])===2 && $m['h1s'][0]==='H' && $m['h1s'][1]==='H' && $m['h1_first']===3 && $m['h1_last']===5) && !($m['h1_first']===3 && $m['h1_last']===5 && $m['h1c']===2 && $m['sc_h']===1 && $m['sc_a']===1 && $m['h1s']===['H','A']) && !($m['h1_first']===3 && $m['h1_last']===7 && $m['h1s']===['H','A','A'])) || ($m['league']==='16min' && in_array(trim($m['away']), $p35_teams) && $m['h1c']>=3 && $m['h1_first']===1 && $m['max_run']<=2 && $m['h1_last']>=5 && $m['min_gap']>=2) || (in_array(trim($m['away']), $p35_teams) && $m['h1c']>=3 && $m['h1_first']===0 && $m['max_run']<=2 && $m['h1_last']>=6 && $m['min_gap']>=2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A') || (trim($m['away'])==='Portugal (V)' && $m['h1c']===1 && $m['h1s']===['A']) || (in_array(trim($m['away']), $p35_teams) && $m['h1c']===3 && $m['h1s']===['H','H','A'] && $m['min_gap']===1)))],
        ...
        ['id'=>'P55', 'label'=>'16min + last gol 1H mnt 8 + AWAY unggul HT, bukan first=3 + scorer HAA + HT 1-2', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['h1_last']===8 && $m['sc_a']>$m['sc_h'] && !($m['h1_first']===3 && $m['sc_h']===1 && $m['sc_a']===2 && $m['h1s']===['H','A','A'])))],
        ...
        ['id'=>'P58', 'label'=>'First goal 1H >=3 + span>=5 + min_gap>=3 + bukan first=4 + span=5 + scorer HH + switches=0, atau 20min + first=2 + span>=5 + min_gap>=3 + (last scorer HOME atau gol 1H>=3) + bukan HH mnt 2-9, atau 20min first=4 + last=6 + min_gap=2 + last HOME, atau 16min + first=1 + span>=5 + min_gap>=3 + max_gap>=4, atau 16min + first=0 + span>=5 + min_gap>=3 + selisih HT>=2, bukan Germany HH 2-0 mnt 0-5, atau 16min single AWAY mnt 1/8 kecuali Germany vs Ukraine mnt 1, atau 15min h1c=3 + first=2 + span=5 + min_gap=1, atau 15min first=0 + last=6 + span=6 + min_gap=2', 'data'=>array_values(array_filter($matches, fn($m) => ($m['h1_first']>=3 && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=3 && !($m['h1_first']===4 && ($m['h1_last']-$m['h1_first'])===5 && $m['h1s']===['H','H'] && $m['switches']===0)) || ($m['league']==='20min' && $m['h1_first']===2 && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=3 && ((count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H') || $m['h1c']>=3) && !($m['h1_last']===9 && $m['h1s']===['H','H'] && $m['switches']===0)) || ($m['league']==='20min' && $m['h1_first']===4 && $m['h1_last']===6 && $m['min_gap']===2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H') || ($m['league']==='16min' && $m['h1_first']===1 && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=3 && $m['max_gap']>=4) || ($m['league']==='16min' && $m['h1_first']===0 && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=3 && abs($m['sc_h']-$m['sc_a'])>=2 && !(trim($m['home'])==='Germany (V)' && $m['sc_h']===2 && $m['sc_a']===0 && $m['h1_last']===5 && $m['h1s']===['H','H'])) || ($m['league']==='16min' && $m['h1c']===1 && $m['sc_h']===0 && $m['sc_a']===1 && in_array($m['h1_first'], [1, 8], true) && !(trim($m['home'])==='Germany (V)' && trim($m['away'])==='Ukraine (V)' && $m['h1_first']===1)) || ($m['league']==='15min' && $m['h1c']===3 && $m['h1_first']===2 && ($m['h1_last']-$m['h1_first'])===5 && $m['min_gap']===1) || ($m['league']==='15min' && $m['h1_first']===0 && $m['h1_last']===6 && ($m['h1_last']-$m['h1_first'])===6 && $m['min_gap']===2)))],
        [
            'id' => 'P59',
        ...
        ['id'=>'P62', 'label'=>'HOME shortlist: Getafe / Osasuna / FC Koln / Lazio / Leicester / Napoli / Sevilla / Udinese (bukan first=1 + last=4 + scorer AAH, bukan Leicester HH 2-0 mnt 0-5, bukan Leicester AA 0-2 mnt 1-5), atau 15min umum: first<=1 + last>=6 + switches>=2 + diff<=2 + last scorer HOME, bukan scorer HHAH, atau 15min first=3 + last=7 + diff=2 + last HOME, atau 15min HH 2-0 mnt 2-4, atau 16min: first<=1 + last>=7 + diff<=2 + last scorer HOME, atau 20min: first<=1 + last>=3 + switches>=2 + HT seri + last scorer HOME, atau 20min AH seri mnt 4-6, atau 20min HH 2-0 mnt 1-4, kecuali Getafe away dengan scorer AHA', 'data'=>array_values(array_filter($matches, fn($m) => (($m['league']==='15min' && in_array(trim($m['home']), $p62_teams) && $m['h1_first']<=1 && $m['h1_last']>=4 && ($m['h1_first']===0 || trim($m['home'])!=='FC Koln (V)') && ($m['switches']>=1 || $m['h1c']<=3 || $m['h1_last']>=7) && !($m['h1_first']===1 && $m['h1_last']===4 && $m['h1s']===['A','A','H']) && !(trim($m['home'])==='Leicester City (V)' && $m['h1_first']===0 && $m['h1_last']===5 && $m['h1s']===['H','H']) && !(trim($m['home'])==='Leicester City (V)' && $m['h1_first']===1 && $m['h1_last']===5 && $m['sc_h']===0 && $m['sc_a']===2 && $m['h1s']===['A','A'])) || ($m['league']==='15min' && $m['h1_first']<=1 && $m['h1_last']>=6 && $m['switches']>=2 && abs($m['sc_h']-$m['sc_a'])<=2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H' && !($m['h1s']===['H','A','H','H']) && !($m['h1s']===['H','H','A','H'])) || ($m['league']==='15min' && $m['h1_first']===3 && $m['h1_last']===7 && abs($m['sc_h']-$m['sc_a'])===2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H') || ($m['league']==='15min' && $m['h1_first']===2 && $m['h1_last']===4 && $m['sc_h']===2 && $m['sc_a']===0 && $m['h1s']===['H','H']) || ($m['league']==='16min' && $m['h1_first']<=1 && $m['h1_last']>=7 && abs($m['sc_h']-$m['sc_a'])<=2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H') || ($m['league']==='20min' && $m['h1_first']<=1 && $m['h1_last']>=3 && $m['switches']>=2 && $m['sc_h']===$m['sc_a'] && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H') || ($m['league']==='20min' && $m['h1_first']===4 && $m['h1_last']===6 && $m['sc_h']===$m['sc_a'] && $m['h1s']===['A','H']) || ($m['league']==='20min' && $m['h1_first']===1 && $m['h1_last']===4 && $m['sc_h']===2 && $m['sc_a']===0 && $m['h1s']===['H','H'])) && !(trim($m['away'])==='Getafe CF (V)' && $m['h1s']===['A','H','A'])))],
        ...
        ['id'=>'P66', 'label'=>'AWAY shortlist: Mainz / Getafe / Lille / Liverpool / Lyon / Juventus / Dortmund / Napoli / Bayern / FC Koln / Chelsea (15min, first<=1, last>=5, Napoli max_run<=2, Chelsea first scorer AWAY, bukan AA 0-2 mnt 0-7, bukan h1c=2 scorer AH span>=6, bukan Lille AAA 0-3 mnt 1-6), atau 15min: first=1 + last=4 + h1c=3 + diff=1 + last scorer AWAY, atau 16min: first=0 + last>=6 + diff<=2 + last scorer AWAY, atau 16min single AWAY mnt 1/8 kecuali Germany vs Ukraine mnt 1, atau 20min: first=0 + last>=7 + switches>=2 + diff<=2 + last scorer AWAY, kecuali Getafe away dengan scorer AHA, bukan 20min AHHA 2-2 mnt 0-7 min_gap=0, bukan 20min AHA mnt 0-4-8', 'data'=>array_values(array_filter($matches, fn($m) => (($m['league']==='15min' && in_array(trim($m['away']), $p66_teams) && $m['h1_first']<=1 && $m['h1_last']>=5 && (trim($m['away'])!=='Napoli (V)' || $m['max_run']<=2) && (trim($m['away'])!=='Chelsea (V)' || (count($m['h1s'])>0 && $m['h1s'][0]==='A')) && !($m['h1_first']===0 && $m['h1_last']===7 && $m['h1c']===2 && $m['sc_h']===0 && $m['sc_a']===2 && $m['h1s']===['A','A']) && !($m['h1c']===2 && $m['h1s']===['A','H'] && ($m['h1_last']-$m['h1_first'])>=6) && !(trim($m['away'])==='Lille OSC (V)' && $m['h1_first']===1 && $m['h1_last']===6 && $m['h1s']===['A','A','A'])) || ($m['league']==='15min' && $m['h1_first']===1 && $m['h1_last']===4 && $m['h1c']===3 && abs($m['sc_h']-$m['sc_a'])===1 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A') || ($m['league']==='16min' && $m['h1_first']===0 && $m['h1_last']>=6 && abs($m['sc_h']-$m['sc_a'])<=2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A') || ($m['league']==='16min' && $m['h1c']===1 && $m['sc_h']===0 && $m['sc_a']===1 && in_array($m['h1_first'], [1, 8], true) && !(trim($m['home'])==='Germany (V)' && trim($m['away'])==='Ukraine (V)' && $m['h1_first']===1)) || ($m['league']==='20min' && $m['h1_first']===0 && $m['h1_last']>=7 && $m['switches']>=2 && abs($m['sc_h']-$m['sc_a'])<=2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A')) && !(trim($m['away'])==='Getafe CF (V)' && $m['h1s']===['A','H','A']) && !($m['league']==='20min' && $m['h1_first']===0 && $m['h1_last']===7 && $m['h1c']===4 && $m['sc_h']===2 && $m['sc_a']===2 && $m['min_gap']===0 && $m['h1s']===['A','H','H','A']) && !($m['league']==='20min' && $m['h1_first']===0 && $m['h1_last']===8 && $m['h1c']===3 && $m['h1s']===['A','H','A'])))],
        ['id'=>'P67', 'label'=>'HOME shortlist: Argentina / Denmark / Germany / Russia / Korea Republic / Croatia / Brazil / Nigeria / Serbia (20min, first<=1, last>=5, bukan HH first=1 last=7 h1c=2, bukan AAHAA first=1 last=10, bukan AHA mnt 0-4-8)', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && in_array(trim($m['home']), $p67_teams) && $m['h1_first']<=1 && $m['h1_last']>=5 && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['H','H']) && !($m['h1_first']===1 && $m['h1_last']===10 && $m['h1s']===['A','A','H','A','A']) && !($m['h1_first']===0 && $m['h1_last']===8 && $m['h1c']===3 && $m['h1s']===['A','H','A'])))],

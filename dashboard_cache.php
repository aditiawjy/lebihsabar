...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P27', 'label'=>'Gol terakhir 1H dicetak AWAY, 16min, max_gap>=3, first!=1, span>=6 + (switches>=1 atau max_gap>=6), bukan HHA 2-1 mnt 0-7 min_gap>=3 max_gap=4', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A' && $m['max_gap']>=3 && $m['h1_first']!=1 && ($m['h1_last']-$m['h1_first'])>=6 && ($m['switches']>=1 || $m['max_gap']>=6) && !($m['h1_first']===0 && $m['h1_last']===7 && $m['h1s']===['H','H','A'] && $m['sc_h']===2 && $m['sc_a']===1 && $m['min_gap']>=3 && $m['max_gap']===4)))],
        ...
        [
            'id'=>'P54',
            'label'=>'20min + AWAY unggul HT + last gol 1H mnt 9 + span>=4 + first>=2 + h1c<=4, bukan first=2 + scorer AAH, bukan AA 0-2 mnt 2-9/4-9, bukan China AA 0-2 mnt 5-9, bukan HAA 1-2 mnt 5-9, bukan AAHA 1-3 mnt 2-9 max_gap=3, bukan AAHA 1-3 mnt 3-9 min_gap=1 max_gap=4, bukan 15min HHA 2-1 mnt 1-4 min_gap=1 max_gap=2',
            'data'=>array_values(array_filter($matches, fn($m) =>
                $m['league']==='20min' && $m['sc_a']>$m['sc_h'] && $m['h1_last']===9 && ($m['h1_last']-$m['h1_first'])>=4 && $m['h1_first']>=2 && $m['h1c']<=4 && !($m['h1_first']===2 && $m['h1s']===['A','A','H']) && !($m['h1_first']===2 && $m['h1_last']===9 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===4 && $m['h1_last']===9 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !(trim($m['home'])==='China (V)' && $m['h1_first']===5 && $m['h1_last']===9 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===5 && $m['h1_last']===9 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['league']==='20min' && $m['h1_first']===2 && $m['h1_last']===9 && $m['h1s']===['A','A','H','A'] && $m['sc_h']===1 && $m['sc_a']===3 && $m['max_gap']===3) && !($m['league']==='20min' && $m['h1_first']===3 && $m['h1_last']===9 && $m['h1s']===['A','A','H','A'] && $m['sc_h']===1 && $m['sc_a']===3 && $m['min_gap']===1 && $m['max_gap']===4) && !($m['league']==='15min' && $m['h1_first']===1 && $m['h1_last']===4 && $m['h1s']===['H','H','A'] && $m['sc_h']===2 && $m['sc_a']===1 && $m['min_gap']===1 && $m['max_gap']===2)
            )),
        ],
        [
            'id'=>'P33',
        ...
        ['id'=>'P42', 'label'=>'20min + first goal >=2 + span >=6 + min_gap>=3, bukan scorer HA, bukan HHA 2-1 mnt 2-9, kecuali max_gap>=7 + h1c=2 + away unggul HT, atau HH 2-0 mnt 2-9, bukan jam 07 AA 0-2 mnt 2-8, bukan HHH 3-0 first>=3 last>=10 max_gap<=4, bukan Minggu HH 2-0 mnt 2-8/2-10, bukan AA 0-2 mnt 4-10, exclude AH 1-1 3-10 gap7', 'data'=>array_values(array_filter($matches, fn($m) => matchesP42($m)))],
        ...
        ['id'=>'P46', 'label'=>'16min + span >=6 + min_gap>=2 + h1c==2 + (first=0 atau switches=0), bukan HH 2-0 mnt 1-7, bukan Minggu HH 2-0 mnt 0-6', 'data'=>array_values(array_filter($matches, fn($m) =>
            $m['league']==='16min' && ($m['h1_last']-$m['h1_first'])>=6 && $m['min_gap']>=2 && $m['h1c']===2 && ($m['h1_first']===0 || $m['switches']===0) && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1s']===['H','H'] && $m['sc_h']===2 && $m['sc_a']===0) && !(($m['kickoff_dow_num'] ?? -1)===0 && $m['h1_first']===0 && $m['h1_last']===6 && $m['h1s']===['H','H'] && $m['sc_h']===2 && $m['sc_a']===0)
        ))],
        ...
        ['id'=>'P51', 'label'=>'16min + switches>=2 + first!=1, bukan AHHAH 3-2 mnt 0-8 min_gap=0, bukan AHAHA 1-4 mnt 2-7 min_gap=0 max_run=3', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['switches']>=2 && $m['h1_first']!=1 && !($m['h1_first']===0 && $m['h1_last']===8 && $m['h1s']===['A','H','H','A','H'] && $m['sc_h']===3 && $m['sc_a']===2 && $m['min_gap']===0) && !($m['h1_first']===2 && $m['h1_last']===7 && $m['h1s']===['A','H','A','A','A'] && $m['sc_h']===1 && $m['sc_a']===4 && $m['min_gap']===0 && $m['max_run']===3)))],
        ['id'=>'P52', 'label'=>'16min + span >=6 + min_gap>=3 + selisih HT>=2, bukan HH 2-0 mnt 1-7, bukan Minggu HH 2-0 mnt 0-6', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && ($m['h1_last']-$m['h1_first'])>=6 && $m['min_gap']>=3 && abs($m['sc_h']-$m['sc_a'])>=2 && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1s']===['H','H'] && $m['sc_h']===2 && $m['sc_a']===0) && !(($m['kickoff_dow_num'] ?? -1)===0 && $m['h1_first']===0 && $m['h1_last']===6 && $m['h1s']===['H','H'] && $m['sc_h']===2 && $m['sc_a']===0)))],
        ...
        [
            'id'=>'P76',
            'label'=>'Experimental 2H: gol 1H>=1 + jam 19 + max_gap>=3 + diff=1, exclude AHH 2-1 4-8 min_gap=1 max_gap=3, tanpa team block',
            'data'=>array_values(array_filter($matches, fn($m) =>
                ($m['h1c'] ?? 0) >= 1
                && (($m['kickoff_hour'] ?? -1) === 19)
                && ($m['max_gap'] ?? -1) >= 3
        ...
        ['id'=>'P64', 'label'=>'AWAY shortlist: Liverpool / Napoli / Bayern / FC Koln / FSV Mainz / Lille (15min, first<=1, last>=4, Napoli khusus max_run<=2, kecuali Napoli h1s=[A,A,H], bukan Lille AAA 0-3 mnt 1-6, bukan Lille HAA 1-2 mnt 0-6, bukan AH 1-1 mnt 1-4/1-5, bukan HA 1-1 mnt 1-7, bukan HHA 2-1 mnt 1-4 min_gap=1 max_gap=2, bukan FC Koln HH 2-0 mnt 0-4, bukan AA 0-2 mnt 1-7, bukan first=0 + HOME unggul HT + last>=6 + max_gap<=3) atau umum: first=1 + last>=7 + scorer AH, bukan h1c=2 last=7 first=1', 'data'=>array_values(array_filter($matches, fn($m) => (($m['league']==='15min' && in_array(trim($m['away']), $p64_teams) && $m['h1_first']<=1 && $m['h1_last']>=4 && (trim($m['away'])!=='Napoli (V)' || $m['max_run']<=2) && !(trim($m['away'])==='Napoli (V)' && $m['h1s']===['A','A','H']) && !(trim($m['away'])==='Lille OSC (V)' && $m['h1_first']===1 && $m['h1_last']===6 && $m['h1s']===['A','A','A']) && !(trim($m['away'])==='Lille OSC (V)' && $m['h1_first']===0 && $m['h1_last']===6 && $m['h1c']===3 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===4 && $m['h1c']===2 && $m['h1s']===['A','H']) && !($m['h1_first']===1 && $m['h1_last']===5 && $m['h1c']===2 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1) && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['H','A'] && $m['sc_h']===1 && $m['sc_a']===1) && !(trim($m['away'])==='FC Koln (V)' && $m['h1c']===2 && $m['sc_h']===2 && $m['sc_a']===0 && $m['h1_first']===0 && $m['h1_last']===4 && $m['h1s']===['H','H']) && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===4 && $m['h1c']===3 && $m['h1s']===['H','H','A'] && $m['sc_h']===2 && $m['sc_a']===1 && $m['min_gap']===1 && $m['max_gap']===2) && !($m['h1_first']===0 && $m['sc_h']>$m['sc_a'] && $m['h1_last']>=6 && $m['max_gap']<=3)) || ($m['league']==='15min' && $m['h1_first']===1 && $m['h1_last']>=7 && count($m['h1s'])>0 && $m['h1s'][0]==='A' && $m['h1s'][count($m['h1s'])-1]==='H' && !($m['h1c']===2 && $m['h1_last']===7 && $m['h1_first']===1))) && !($m['league']==='15min' && $m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['H','A'] && $m['sc_h']===1 && $m['sc_a']===1)))],
        [
            'id'=>'P67',
        ...
        ['id'=>'P70', 'label'=>'AWAY single: Liverpool (15min, first<=1, last>=5), kecuali first=0 + HOME unggul HT + max_gap<=3', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='15min' && trim($m['away'])==='Liverpool (V)' && $m['h1_first']<=1 && $m['h1_last']>=5 && !($m['h1_first']===0 && $m['sc_h']>$m['sc_a'] && $m['max_gap']<=3)))],
        ...
        ['id'=>'P79', 'label'=>'Structural no-team: 16min + switches=1 + first<=1 + max_gap>=5, tanpa exact team/block team', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['switches']===1 && $m['h1_first']<=1 && $m['max_gap']>=5))],
        ['id'=>'P80', 'label'=>'Structural no-team: 20min + scorer HAA + span>=5 + min_gap>=2, exclude HAA 1-2 1-8, tanpa exact team/block team', 'data'=>array_values(array_filter($matches, fn($m) => !isP81P82P83StructuralMiss($m) && $m['league']==='20min' && $m['h1s']===['H','A','A'] && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=2))],
        ['id'=>'P81', 'label'=>'High-hit no-team union: 16min switches=1 first<=1 max_gap>=5, atau 20min HAA span>=5 min_gap>=2, atau 20min first=1 last=8 switches>=2, atau AAA first>=2 max_gap>=4, atau 20min h1c=2 first=3 span>=5, atau 15min 2-1 max_gap=4 last>=5, atau 20min 1-2 min_gap=3, atau 3H A-start span=6 max_gap=4, atau 16min 1-1 first<=1 span>=4, exclude HAA 1-2 1-8 + AH 1-1 gap7, tanpa exact team/block team', 'data'=>array_values(array_filter($matches, fn($m) => !isP82StructuralMiss($m) && (
        ...
        ['id'=>'P83', 'label'=>'No-team union v3 non-0-0 2H: P82 plus 20min first=5 max_gap>=3 sw>=1; HT 2-2 last<=6 min_gap=0 max_gap<=4; first>=1 min_gap=1 max_gap>=4 sw=0; 16min h1c>=3 max_gap<=2 diff<=1; 16min h1c>=3 last<=5 last scorer A; h1c>=2 first=7 last>=8 first scorer H; first=0 last<=2 max_run=2 last scorer A; exclude AH gap7 + HAH 2-1 3-5, tanpa HT 0-0, tanpa team block/exact team', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1c']>0 && !isP83StructuralMiss($m) && (
            ($m['league']==='16min' && $m['switches']===1 && $m['h1_first']<=1 && $m['max_gap']>=5)
            || ($m['league']==='20min' && $m['h1s']===['H','A','A'] && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=2)
            || ($m['league']==='20min' && $m['h1_first']===1 && $m['h1_last']===8 && $m['switches']>=2)
            || ($m['h1s']===['A','A','A'] && $m['h1_first']>=2 && $m['max_gap']>=4)

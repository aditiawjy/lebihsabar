...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P3',  'label'=>'AH gap >=3 mnt + lm>=6, 15min/16min', 'data'=>array_values(array_filter($matches, fn($m) => in_array($m['league'],['15min','16min']) && $m['h1c']==2 && $m['sc_h']==1 && $m['sc_a']==1 && $m['h1s']==['A','H'] && ($m['h1'][1]['min']-$m['h1'][0]['min']) >= 3 && $m['h1_last']>=6))],
        ...
        ['id'=>'P32', 'label'=>'Span 1H >=9 mnt + gol >=2 + selisih <=3, min_gap>=3, 20min', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['h1c']>=2 && ($m['h1_last']-$m['h1_first'])>=9 && abs($m['sc_h']-$m['sc_a'])<=3 && $m['min_gap']>=3))],

...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P53', 'label'=>'20min + last gol 1H mnt 3 + last scorer HOME + min_gap>=1 + gol 1H<=2 + (first=0 atau away HT=0), atau AH 1-1 mnt 1-3 kecuali Paraguay vs Bosnia-Herzegovina, atau 20min + last=4 + last scorer AWAY + min_gap>=1 + gol 1H<=2 + (first=0 atau away HT=0), kecuali Greece vs Ukraine dan Egypt vs Australia', 'data'=>array_values(array_filter($matches, fn($m) => (($m['league']==='20min' && $m['h1_last']===3 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H' && $m['min_gap']>=1 && $m['h1c']<=2 && ($m['h1_first']===0 || $m['sc_a']===0)) || ($m['league']==='20min' && $m['h1_first']===1 && $m['h1_last']===3 && $m['h1c']===2 && $m['sc_h']===1 && $m['sc_a']===1 && $m['h1s']===['A','H'] && !(trim($m['home'])==='Paraguay (V)' && trim($m['away'])==='Bosnia-Herzegovina (V)')) || ($m['league']==='20min' && $m['h1_last']===4 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A' && $m['min_gap']>=1 && $m['h1c']<=2 && ($m['h1_first']===0 || $m['sc_a']===0))) && !(trim($m['home'])==='Greece (V)' && trim($m['away'])==='Ukraine (V)') && !(trim($m['home'])==='Egypt (V)' && trim($m['away'])==='Australia (V)')))],
        ...
        ['id'=>'P62', 'label'=>'HOME shortlist: Getafe / Osasuna / FC Koln / Lazio / Leicester / Napoli / Sevilla / Udinese (bukan first=1 + last=4 + scorer AAH), atau 15min umum: first<=1 + last>=6 + switches>=2 + diff<=2 + last scorer HOME, atau 16min: first<=1 + last>=7 + diff<=2 + last scorer HOME, atau 20min: first<=1 + last>=3 + switches>=2 + HT seri + last scorer HOME, kecuali Getafe away dengan scorer AHA', 'data'=>array_values(array_filter($matches, fn($m) => (($m['league']==='15min' && in_array(trim($m['home']), $p62_teams) && $m['h1_first']<=1 && $m['h1_last']>=4 && ($m['h1_first']===0 || trim($m['home'])!=='FC Koln (V)') && ($m['switches']>=1 || $m['h1c']<=3 || $m['h1_last']>=7) && !($m['h1_first']===1 && $m['h1_last']===4 && $m['h1s']===['A','A','H'])) || ($m['league']==='15min' && $m['h1_first']<=1 && $m['h1_last']>=6 && $m['switches']>=2 && abs($m['sc_h']-$m['sc_a'])<=2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H') || ($m['league']==='16min' && $m['h1_first']<=1 && $m['h1_last']>=7 && abs($m['sc_h']-$m['sc_a'])<=2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H') || ($m['league']==='20min' && $m['h1_first']<=1 && $m['h1_last']>=3 && $m['switches']>=2 && $m['sc_h']===$m['sc_a'] && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H')) && !(trim($m['away'])==='Getafe CF (V)' && $m['h1s']===['A','H','A'])))],

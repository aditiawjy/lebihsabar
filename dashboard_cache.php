...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P41', 'label'=>'Selisih HT >=2 + first goal >=2 + span >=6 + max_gap>=5, kecuali h1c=2 + max_gap>=7 + away unggul HT, atau 20min HH 2-0 mnt 2-9, bukan 20min HHH first>=2 last>=10', 'data'=>array_values(array_filter($matches, fn($m) => abs($m['sc_h']-$m['sc_a'])>=2 && $m['h1_first']>=2 && ($m['h1_last']-$m['h1_first'])>=6 && $m['max_gap']>=5 && !($m['h1c']===2 && $m['max_gap']>=7 && $m['sc_a']>$m['sc_h']) && !($m['league']==='20min' && $m['h1c']===2 && $m['sc_h']===2 && $m['sc_a']===0 && $m['h1_first']===2 && $m['h1_last']===9 && $m['h1s']===['H','H']) && !($m['league']==='20min' && $m['h1_first']>=2 && $m['h1_last']>=10 && $m['h1s']===['H','H','H'])))],
        ...
        [
            'id'=>'P58',
            'label'=>'First goal 1H >=3 + span>=5 + min_gap>=3 + bukan first=4 + span=5 + scorer HH + switches=0, atau 20min + first=2 + span>=5 + min_gap>=3 + (last scorer HOME atau gol 1H>=3) + bukan HH mnt 2-9, atau 16min + first=1 + span>=5 + min_gap>=3 + max_gap>=4, atau 16min + first=0 + span>=5 + min_gap>=3 + selisih HT>=2, bukan Netherlands AH 1-1 mnt 4-9, bukan 20min HH 2-0 mnt 2-7, bukan 20min HHA 2-1 mnt 2-9, bukan 20min AA 0-2 mnt 4-9, bukan HH 2-0 mnt 0-5/1-7, bukan 16min single AWAY mnt 1/8, bukan 15min HAA 1-2 mnt 2-7, bukan 16min AH 1-1 mnt 3-8, atau 15min h1c=3 + first=2 + span=5 + min_gap=1, atau 15min first=0 + last=6 + span=6 + min_gap=2',
            'data'=>array_values(array_filter($matches, fn($m) =>
                (
                ...
                && !(trim($m['home'])==='Netherlands (V)' && $m['league']==='20min' && $m['h1_first']===4 && $m['h1_last']===9 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1)
        ...
        [
            'id'=>'P61',
            'label'=>'AWAY shortlist: Chelsea / Lille / Juventus / Monaco (15min, last>=5, diff<=1, kecuali Bordeaux vs Lille dan Monaco HA 1-1 mnt 3-5, bukan single goal mnt 5, bukan single AWAY 0-1 mnt 6, bukan AH 1-1 mnt 4-6, bukan AH 1-1 mnt 3-7, bukan AHA 1-2 last=7 min_gap=0, bukan HAA 1-2 mnt 0-5, bukan HAA 1-2 mnt 5-7, bukan HHH 3-0 mnt 1-5, bukan AHH 2-1 mnt 1-6, bukan Lille HAA 1-2 mnt 0-6, bukan HAA 1-2 mnt 2-7) atau umum: first<=1 + last>=6 + first&last scorer HOME + diff<=1',
            'data'=>array_values(array_filter($matches, fn($m) =>
                ($m['league']==='15min' && in_array(trim($m['away']), $p61_teams) && $m['h1_last']>=5 && abs($m['sc_h']-$m['sc_a'])<=1 && !(trim($m['home'])==='Girondins de Bordeaux (V)' && trim($m['away'])==='Lille OSC (V)') && !(trim($m['away'])==='AS Monaco (V)' && $m['h1_first']===3 && $m['h1_last']===5 && $m['h1c']===2 && $m['h1s']===['H','A']) && !($m['h1c']===1 && $m['h1_last']===5) && !($m['h1c']===1 && $m['h1_first']===6 && $m['h1_last']===6 && $m['h1s']===['A'] && $m['fh']===0 && $m['fa']===1) && !($m['h1_first']===4 && $m['h1_last']===6 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1) && !($m['h1_first']===3 && $m['h1_last']===7 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1) && !($m['h1_last']===7 && $m['min_gap']===0 && $m['h1s']===['A','H','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===0 && $m['h1_last']===5 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===5 && $m['h1_last']===7 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===5 && $m['h1s']===['H','H','H'] && $m['sc_h']===3 && $m['sc_a']===0) && !($m['h1_first']===1 && $m['h1_last']===6 && $m['h1s']===['A','H','H'] && $m['sc_h']===2 && $m['sc_a']===1) && !(trim($m['away'])==='Lille OSC (V)' && $m['h1_first']===0 && $m['h1_last']===6 && $m['h1c']===3 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===2 && $m['h1_last']===7 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2))
                || ($m['league']==='15min' && $m['h1_first']<=1 && $m['h1_last']>=6 && abs($m['sc_h']-$m['sc_a'])<=1 && count($m['h1s'])>0 && $m['h1s'][0]==='H' && $m['h1s'][count($m['h1s'])-1]==='H')
            )),
        ],
        [
            'id'=>'P62',
            'label'=>'HOME shortlist: Getafe / Osasuna / FC Koln / Lazio / Leicester / Napoli / Sevilla / Udinese (bukan first=1 + last=4 + scorer AAH, bukan Leicester HH 2-0 mnt 0-5, bukan Leicester AA 0-2 mnt 1-5, bukan AA 0-2 mnt 0-5), atau 15min umum: first<=1 + last>=6 + switches>=2 + diff<=2 + last scorer HOME, bukan scorer HHAH, atau 15min first=3 + last=7 + diff=2 + last HOME, atau 15min HH 2-0 mnt 2-4, atau 16min: first<=1 + last>=7 + diff<=2 + last scorer HOME, atau 20min: first<=1 + last>=3 + switches>=2 + HT seri + last scorer HOME, atau 20min HH 2-0 mnt 1-4, kecuali Getafe away dengan scorer AHA, bukan Lazio HH 2-0 mnt 2-4, bukan 15min h1c=5 + HT 3-2 + first=0 + last>=7 + min_gap=0, bukan 16min HH 2-0 mnt 1-7, bukan 16min AHHAH 3-2 mnt 0-8, bukan 20min HAAH 2-2 mnt 0-5',
            'data'=>array_values(array_filter($matches, fn($m) =>
        ...
        [
            'id'=>'P65',
            'label'=>'HOME shortlist: Leicester / Napoli / Udinese / Lyon (15min, gol 1H>=1, first<=1, bukan Leicester HH 2-0 mnt 0-5, bukan Leicester AA 0-2 mnt 1-5, bukan Napoli single AWAY mnt 1, bukan Lyon single HOME mnt 1, bukan HH 2-0 mnt 1-3, bukan AA 0-2 mnt 1-3)',
            'data'=>array_values(array_filter($matches, fn($m) =>
                $m['league']==='15min' && in_array(trim($m['home']), $p65_teams) && $m['h1c']>=1 && $m['h1_first']<=1 && !(trim($m['home'])==='Leicester City (V)' && $m['h1_first']===0 && $m['h1_last']===5 && $m['h1s']===['H','H']) && !(trim($m['home'])==='Leicester City (V)' && $m['h1_first']===1 && $m['h1_last']===5 && $m['sc_h']===0 && $m['sc_a']===2 && $m['h1s']===['A','A']) && !(trim($m['home'])==='Napoli (V)' && $m['h1c']===1 && $m['h1_first']===1 && $m['sc_h']===0 && $m['sc_a']===1 && $m['h1s']===['A']) && !(trim($m['home'])==='Olympique Lyonnais (V)' && $m['h1c']===1 && $m['h1_first']===1 && $m['sc_h']===1 && $m['sc_a']===0 && $m['h1s']===['H']) && !($m['h1_first']===1 && $m['h1_last']===3 && $m['h1s']===['H','H'] && $m['sc_h']===2 && $m['sc_a']===0) && !($m['h1_first']===1 && $m['h1_last']===3 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2)
            )),
        ],
        [
            'id'=>'P66',
        ...
        [
            'id'=>'P67',
            'label'=>'HOME shortlist: Argentina / Denmark / Germany / Russia / Korea Republic / Croatia / Brazil / Nigeria / Serbia / USA / South Africa / Portugal / Oman / New Zealand / Philippines / Iran / Iraq (20min, first<=1, last>=5, bukan HH first=1 last=7 h1c=2, bukan HH 2-0 first=1 last=6, bukan AAHAA first=1 last=10, bukan AAA 0-3 mnt 1-8, bukan AHA mnt 0-4-8, bukan AAH 1-2 mnt 0-5, bukan AHA 1-2 mnt 0-7, bukan AA 0-2 mnt 0-6, bukan AA 0-2 mnt 1-5, bukan AA 0-2 mnt 1-9, bukan HAAH 2-2 mnt 0-5)',
            'data'=>array_values(array_filter($matches, fn($m) =>
                $m['league']==='20min' && in_array(trim($m['home']), $p67_teams) && $m['h1_first']<=1 && $m['h1_last']>=5 && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['H','H']) && !($m['h1_first']===1 && $m['h1_last']===6 && $m['h1c']===2 && $m['h1s']===['H','H'] && $m['sc_h']===2 && $m['sc_a']===0) && !($m['h1_first']===1 && $m['h1_last']===10 && $m['h1s']===['A','A','H','A','A']) && !($m['h1_first']===1 && $m['h1_last']===8 && $m['h1s']===['A','A','A'] && $m['sc_h']===0 && $m['sc_a']===3) && !($m['h1_first']===0 && $m['h1_last']===8 && $m['h1c']===3 && $m['h1s']===['A','H','A']) && !($m['h1_first']===0 && $m['h1_last']===5 && $m['h1c']===3 && $m['h1s']===['A','A','H'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===0 && $m['h1_last']===7 && $m['h1c']===3 && $m['h1s']===['A','H','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===0 && $m['h1_last']===6 && $m['h1c']===2 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===5 && $m['h1c']===2 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===9 && $m['h1c']===2 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===0 && $m['h1_last']===5 && $m['h1s']===['H','A','A','H'] && $m['sc_h']===2 && $m['sc_a']===2)
            )),
        ],

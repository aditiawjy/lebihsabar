...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        [
            'id'=>'P61',
            'label'=>'AWAY shortlist: Chelsea / Lille / Juventus / Monaco (15min, last>=5, diff<=1, tanpa exact-team block; bukan single goal mnt 5/6, bukan HA/AH 1-1 mnt 3-5/4-6/3-7, bukan AHA 1-2 last=7 min_gap=0, bukan HAA 1-2 mnt 0-6/2-7/5-7, bukan HHH 3-0 mnt 1-5, bukan AHH 2-1 mnt 1-6, bukan HA 1-1 mnt 6 min_gap=0, bukan AAH 1-2 mnt 3-7, bukan HAHA 2-2 mnt 2-6 max_gap<=2) atau umum: first<=1 + last>=6 + first&last scorer HOME + diff<=1',
            'data'=>array_values(array_filter($matches, fn($m) =>
        ...
        [
            'id'=>'P62',
            'label'=>'HOME shortlist: Getafe / Osasuna / FC Koln / Lazio / Leicester / Napoli / Sevilla / Udinese (bukan first=1 + last=4 + scorer AAH, bukan Leicester HH 2-0 mnt 0-5, bukan Leicester AA 0-2 mnt 1-5, bukan AA 0-2 mnt 0-5, bukan 15min AA 0-2 mnt 1-5), atau 15min umum: first<=1 + last>=6 + switches>=2 + diff<=2 + last scorer HOME, bukan scorer HHAH, atau 15min first=3 + last=7 + diff=2 + last HOME, atau 15min HH 2-0 mnt 2-4, atau 16min: first<=1 + last>=7 + diff<=2 + last scorer HOME, atau 20min: first<=1 + last>=3 + switches>=2 + HT seri + last scorer HOME, atau 20min HH 2-0 mnt 1-4, kecuali Getafe away dengan scorer AHA, bukan AHA 1-2 mnt 1-7 min_gap=1 max_gap=5, bukan Lazio HH 2-0 mnt 2-4, bukan 15min h1c=5 + HT 3-2 + first=0 + last>=7 + min_gap=0, bukan 15min HAAA 1-3 mnt 0-8, bukan 15min HAHH 3-1 mnt 0-4 min_gap=0 max_gap=3, bukan Minggu 15min AA 0-2 mnt 1-4, bukan 15min HH 2-0 mnt 2-4 hari Minggu/jam 07, bukan 16min HH 2-0 mnt 1-7, bukan 16min AHHAH 3-2 mnt 0-8, bukan Minggu 20min HAAH 2-2 mnt 0-4/0-5, bukan 15min HH 2-0 mnt 0-7',
            'data'=>array_values(array_filter($matches, fn($m) =>
        ...
        [
            'id'=>'P72',
            'label'=>'Structural HOME P62 tanpa team: 15min first<=1 + last>=6 + switches>=2 + diff<=2 + last HOME, atau 15min first=3 last=7 diff=2 last HOME, atau 15min HH 2-0 mnt 2-4, atau 16min first<=1 last>=7 diff<=2 last HOME, atau 20min first<=1 last>=3 switches>=2 HT seri last HOME, atau 20min HH 2-0 mnt 1-4; bukan HAHH/HHAH, bukan 15min HT 3-2 first=0 last>=7 min_gap=0, bukan Minggu/jam 07 15min HH 2-0 mnt 2-4, bukan 16min HH 2-0 mnt 1-7, bukan 16min AHHAH 3-2 mnt 0-8, bukan 20min HAAH 2-2 mnt 0-5',
        ...
        ['id'=>'P64', 'label'=>'AWAY shortlist: Liverpool / Napoli / Bayern / FC Koln / FSV Mainz / Lille (15min, first<=1, last>=4, Napoli khusus max_run<=2, kecuali Napoli h1s=[A,A,H], bukan Lille AAA 0-3 mnt 1-6, bukan Lille HAA 1-2 mnt 0-6, bukan AH 1-1 mnt 1-4/1-5, bukan HA 1-1 mnt 1-7, bukan HHA 2-1 mnt 1-4 min_gap=1 max_gap=2, bukan FC Koln HH 2-0 mnt 0-4, bukan AA 0-2 mnt 1-7) atau umum: first=1 + last>=7 + scorer AH, bukan h1c=2 last=7 first=1', 'data'=>array_values(array_filter($matches, fn($m) => (($m['league']==='15min' && in_array(trim($m['away']), $p64_teams) && $m['h1_first']<=1 && $m['h1_last']>=4 && (trim($m['away'])!=='Napoli (V)' || $m['max_run']<=2) && !(trim($m['away'])==='Napoli (V)' && $m['h1s']===['A','A','H']) && !(trim($m['away'])==='Lille OSC (V)' && $m['h1_first']===1 && $m['h1_last']===6 && $m['h1s']===['A','A','A']) && !(trim($m['away'])==='Lille OSC (V)' && $m['h1_first']===0 && $m['h1_last']===6 && $m['h1c']===3 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===4 && $m['h1c']===2 && $m['h1s']===['A','H']) && !($m['h1_first']===1 && $m['h1_last']===5 && $m['h1c']===2 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1) && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['H','A'] && $m['sc_h']===1 && $m['sc_a']===1) && !(trim($m['away'])==='FC Koln (V)' && $m['h1c']===2 && $m['sc_h']===2 && $m['sc_a']===0 && $m['h1_first']===0 && $m['h1_last']===4 && $m['h1s']===['H','H']) && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===4 && $m['h1c']===3 && $m['h1s']===['H','H','A'] && $m['sc_h']===2 && $m['sc_a']===1 && $m['min_gap']===1 && $m['max_gap']===2)) || ($m['league']==='15min' && $m['h1_first']===1 && $m['h1_last']>=7 && count($m['h1s'])>0 && $m['h1s'][0]==='A' && $m['h1s'][count($m['h1s'])-1]==='H' && !($m['h1c']===2 && $m['h1_last']===7 && $m['h1_first']===1))) && !($m['league']==='15min' && $m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['H','A'] && $m['sc_h']===1 && $m['sc_a']===1)))],
        [
            'id'=>'P65',

...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P22', 'label'=>'Away menang HT, 16min, gol>=2, span>=3, balas>=1 + switches>=2', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['sc_a'] > $m['sc_h'] && $m['h1c']>=2 && ($m['h1_last']-$m['h1_first'])>=3 && $m['switches']>=2))],
...
function computeNextPatterns(array $matches): array {
    return [
        ...
        ['id'=>'NG5','label'=>'Seri 1-1 + scorer HA + gol terakhir mnt 7 + fm>=2 + gap>=4','next'=>'AWAY','data'=>array_values(array_filter($matches, fn($m) => $m['sc_h']==1 && $m['sc_a']==1 && $m['h1c']==2 && count($m['h1s'])>=2 && $m['h1s'][0]==='H' && $m['h1s'][count($m['h1s'])-1]==='A' && $m['h1_last']==7 && $m['h1_first']>=2 && $m['max_gap']>=4))],

...
function isP82StructuralMiss(array $m): bool {
    ...
    // Ambiguous signature: 16min AH 1-1 mnt 1-6 — fingerprint babak-1 identik antara hit & miss,
    // tidak ada pembeda struktural jujur, jadi seluruh signature di-drop (bukan diklaim).
    if (($m['league'] ?? '') === '16min'
        && ($m['h1s'] ?? []) === ['A','H']
        && ($m['sc_h'] ?? 0) === 1 && ($m['sc_a'] ?? 0) === 1
        && ($m['h1_first'] ?? -1) === 1 && ($m['h1_last'] ?? -1) === 6) {
        return true;
    }

    // Clean miss: 16min AHA 1-2 mnt 4-7 (tidak ada hit dengan signature ini)
    if (($m['league'] ?? '') === '16min'
        && ($m['h1s'] ?? []) === ['A','H','A']
        && ($m['sc_h'] ?? 0) === 1 && ($m['sc_a'] ?? 0) === 2
        && ($m['h1_first'] ?? -1) === 4 && ($m['h1_last'] ?? -1) === 7) {
        return true;
    }
...
function isP83StructuralMiss(array $m): bool {
    ...
    // Clean miss: 20min AA 0-2 mnt 0-2 (tidak ada hit dengan signature ini)
    if (($m['league'] ?? '') === '20min'
        && ($m['h1s'] ?? []) === ['A','A']
        && ($m['sc_h'] ?? 0) === 0 && ($m['sc_a'] ?? 0) === 2
        && ($m['h1_first'] ?? -1) === 0 && ($m['h1_last'] ?? -1) === 2) {
        return true;
    }
...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        [
            'id' => 'P6',
            'label' => 'Seri 1-1 + gol penyama mnt 7\' + span>=5 + first!=1, tanpa team block/exact match; exclude struktur jam 14:01, 22:46, 23:02, 20:31',
        ...
        [
            'id' => 'P12',
            'label' => 'Total gol 1H >= 4 + span >= 6 mnt + min_gap>=1 + lm<=9 + first>=2 + max_run<=2 + struktur multi-branch (HAHA/AHAH/AHHH/HHAA/HAAH/AHAA/HHHA/AAAH/AAAA), bukan 20min AAAH 1-3 mnt 0-10 min_gap=0, bukan 20min AAHA 1-3 mnt 2-9 max_gap=3, tanpa team block',
        ...
        ['id'=>'P15', 'label'=>'HT 2-2 + max_gap<=2 + bukan last=8 atau (first=0 dan last=5), bukan 15min AHHA mnt 1-2-4-6, bukan 20min HAAH 2-2 mnt 0-5, bukan 15min HAHA 2-2 mnt 2-6 min_gap=1 max_gap=2, atau 20min + HT 2-2 + max_gap=3 + first=0 + last<=7, atau 16min + HT 2-2 + max_gap=3 + first>=2, atau 15min + HT 2-2 + max_gap=3 + first=1 + last<=6', 'data'=>array_values(array_filter($matches, fn($m) => (($m['sc_h']==2 && $m['sc_a']==2 && $m['max_gap']<=2 && $m['h1_last']!=8 && !($m['h1_first']===0 && $m['h1_last']===5)) || ($m['league']==='20min' && $m['sc_h']==2 && $m['sc_a']==2 && $m['max_gap']===3 && $m['h1_first']===0 && $m['h1_last']<=7) || ($m['league']==='16min' && $m['sc_h']==2 && $m['sc_a']==2 && $m['max_gap']===3 && $m['h1_first']>=2) || ($m['league']==='15min' && $m['sc_h']==2 && $m['sc_a']==2 && $m['max_gap']===3 && $m['h1_first']===1 && $m['h1_last']<=6)) && !($m['league']==='15min' && $m['h1s']===['A','H','H','A'] && array_map(fn($g) => $g['min'], $m['h1'])===[1,2,4,6]) && !($m['league']==='20min' && $m['h1_first']===0 && $m['h1_last']===5 && $m['h1s']===['H','A','A','H'] && $m['sc_h']===2 && $m['sc_a']===2) && !($m['league']==='15min' && $m['h1_first']===2 && $m['h1_last']===6 && $m['h1s']===['H','A','H','A'] && $m['sc_h']===2 && $m['sc_a']===2 && $m['min_gap']===1 && $m['max_gap']===2)))],
        ...
        ['id'=>'P26', 'label'=>'HT total ganjil (1,3,5...), 16min, last mnt >=6, gol 1H >=2, away unggul + max_run<=2 + (first!=1 atau max_gap>=4), bukan HAA 1-2 mnt 3-8/5-8, bukan AHA 1-2 mnt 4-7', 'data'=>array_values(array_filter($matches, fn($m) =>
            $m['league']==='16min' && ($m['sc_h']+$m['sc_a'])%2===1 && $m['h1_last']>=6 && $m['h1c']>=2 && $m['sc_a']>$m['sc_h'] && $m['max_run']<=2 && ($m['h1_first']!=1 || $m['max_gap']>=4) && !($m['h1_last']===8 && in_array($m['h1_first'], [3,5], true) && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===4 && $m['h1_last']===7 && $m['h1s']===['A','H','A'] && $m['sc_h']===1 && $m['sc_a']===2)
        ))],
        ['id'=>'P27', 'label'=>'Gol terakhir 1H dicetak AWAY, 16min, max_gap>=3, first!=1, span>=6 + (switches>=1 atau max_gap>=6), bukan HHA 2-1 mnt 0-7 min_gap>=3 max_gap=4, kecuali first=2 + last=8 + h1c=2 + max_gap=6', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A' && $m['max_gap']>=3 && $m['h1_first']!=1 && ($m['h1_last']-$m['h1_first'])>=6 && ($m['switches']>=1 || $m['max_gap']>=6) && !($m['h1_first']===0 && $m['h1_last']===7 && $m['h1s']===['H','H','A'] && $m['sc_h']===2 && $m['sc_a']===1 && $m['min_gap']>=3 && $m['max_gap']===4) && !($m['h1_first']===2 && $m['h1_last']===8 && $m['h1c']===2 && $m['max_gap']===6)))],
        ...
        ['id'=>'P40', 'label'=>'16min + selisih HT tepat 2 + first goal <=1 + span>=5 + (min_gap>=1 atau max_gap>=4), bukan HH 2-0 mnt 0-5/1-7, bukan last AWAY + home lead>=2 + max_run>=3 + max_gap<=3, bukan jam 18 + AAHA 1-3 mnt 1-6 max_gap<=2, bukan Minggu HH 2-0 mnt 0-6', 'data'=>array_values(array_filter($matches, fn($m) =>
        ...
        ['id'=>'P51', 'label'=>'16min + switches>=2 + first!=1, bukan AHHAH 3-2 mnt 0-8 min_gap=0, bukan AHAHA 1-4 mnt 2-7 min_gap=0 max_run=3, bukan AHA 1-2 mnt 4-7, kecuali first>=3 + last<=5', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['switches']>=2 && $m['h1_first']!=1 && !($m['h1_first']===0 && $m['h1_last']===8 && $m['h1s']===['A','H','H','A','H'] && $m['sc_h']===3 && $m['sc_a']===2 && $m['min_gap']===0) && !($m['h1_first']===2 && $m['h1_last']===7 && $m['h1s']===['A','H','A','A','A'] && $m['sc_h']===1 && $m['sc_a']===4 && $m['min_gap']===0 && $m['max_run']===3) && !($m['h1_first']===4 && $m['h1_last']===7 && $m['h1s']===['A','H','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']>=3 && $m['h1_last']<=5)))],
        ...
        ['id'=>'P55', 'label'=>'16min + last gol 1H mnt 8 + AWAY unggul HT, bukan first=3 + scorer HAA + HT 1-2, bukan HAA 1-2 mnt 5-8 min_gap=0, bukan single AWAY mnt 8, kecuali h1c=2 + max_gap=6 + min_gap=6', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['h1_last']===8 && $m['sc_a']>$m['sc_h'] && !($m['h1_first']===3 && $m['sc_h']===1 && $m['sc_a']===2 && $m['h1s']===['H','A','A']) && !($m['h1_first']===5 && $m['h1_last']===8 && $m['sc_h']===1 && $m['sc_a']===2 && $m['h1s']===['H','A','A'] && $m['min_gap']===0) && !($m['h1c']===1 && $m['h1_first']===8 && $m['h1s']===['A'] && $m['sc_h']===0 && $m['sc_a']===1) && !($m['h1c']===2 && $m['max_gap']===6 && $m['min_gap']===6)))],
        ...
        [
            ...
            'data'=>array_values(array_filter($matches, fn($m) =>
                ...
                && !($m['league']==='16min' && $m['h1_first']===1 && $m['h1_last']===6 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1)
        ...
        [
            'id'=>'P61',
            'label'=>'AWAY shortlist: Chelsea / Lille / Juventus / Monaco (15min, last>=5, diff<=1, tanpa exact-team block; bukan single goal mnt 5/6, bukan HA/AH 1-1 mnt 3-5/4-6/3-7, bukan AHA 1-2 last=7 min_gap=0, bukan HAA 1-2 mnt 0-6/2-7/5-7, bukan HHH 3-0 mnt 1-5, bukan AHH 2-1 mnt 1-6, bukan HA 1-1 mnt 6 min_gap=0, bukan AAH 1-2 mnt 3-7, bukan HAHA 2-2 mnt 2-6 max_gap<=2) atau umum: first<=1 + last>=6 + first&last scorer HOME + diff<=1, bukan 15min HAH 2-1 mnt 0-6 min_gap=3 max_gap=3, exclude AH 1-1 gap7',
            'data'=>array_values(array_filter($matches, fn($m) =>
        ...
        [
            'id'=>'P72',
            'label'=>'Structural HOME tanpa team: 15min first<=1 + last>=6 + switches>=2 + diff<=2 + last HOME, atau 15min first=3 last=7 diff=2 last HOME, atau 15min HH 2-0 mnt 2-4, atau 16min first<=1 last>=7 diff<=2 last HOME, atau 20min first<=1 last>=3 switches>=2 HT seri last HOME, atau 20min HH 2-0 mnt 1-4; bukan HAHH/HHAH, bukan 15min HT 3-2 first=0 last>=7 min_gap=0, bukan Minggu/jam 07/jam 08 menit>=45/jam 11 15min HH 2-0 mnt 2-4, bukan 16min HH 2-0 mnt 1-7, bukan 16min AHHAH 3-2 mnt 0-8, bukan 20min HAAH 2-2 mnt 0-5, bukan 15min HAH 2-1 mnt 0-6 min_gap=3 max_gap=3, bukan 15min HAAH 2-2 mnt 1-6, exclude AH 1-1 gap7',
            'data'=>array_values(array_filter($matches, function($m) {
                ...
                return $base
                    ...
                    && !($m['league']==='15min' && $m['h1_first']===1 && $m['h1_last']===6 && $m['h1s']===['H','A','A','H'] && $m['sc_h']===2 && $m['sc_a']===2);
            })),
        ],
        [
            'id'=>'P73',
        ...
        ['id'=>'P64', 'label'=>'AWAY shortlist: Liverpool / Napoli / Bayern / FC Koln / FSV Mainz / Lille (15min, first<=1, last>=4, Napoli khusus max_run<=2, kecuali Napoli h1s=[A,A,H], bukan Lille AAA 0-3 mnt 1-6, bukan Lille HAA 1-2 mnt 0-6, bukan AH 1-1 mnt 1-4/1-5, bukan HA 1-1 mnt 1-7, bukan HHA 2-1 mnt 1-4 min_gap=1 max_gap=2, bukan FC Koln HH 2-0 mnt 0-4, bukan AA 0-2 mnt 1-7, bukan first=0 + HOME unggul HT + last>=6 + max_gap<=3, bukan first<=1 + last=5 + max_gap=4 + kickoff jam 20/21) atau umum: first=1 + last>=7 + scorer AH, bukan h1c=2 last=7 first=1', 'data'=>array_values(array_filter($matches, fn($m) => !(($m['h1_first']??99)<=1 && ($m['h1_last']??-1)===5 && ($m['max_gap']??-1)===4 && in_array(($m['kickoff_hour']??99), [20,21], true)) && (($m['league']==='15min' && in_array(trim($m['away']), $p64_teams) && $m['h1_first']<=1 && $m['h1_last']>=4 && (trim($m['away'])!=='Napoli (V)' || $m['max_run']<=2) && !(trim($m['away'])==='Napoli (V)' && $m['h1s']===['A','A','H']) && !(trim($m['away'])==='Lille OSC (V)' && $m['h1_first']===1 && $m['h1_last']===6 && $m['h1s']===['A','A','A']) && !(trim($m['away'])==='Lille OSC (V)' && $m['h1_first']===0 && $m['h1_last']===6 && $m['h1c']===3 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===4 && $m['h1c']===2 && $m['h1s']===['A','H']) && !($m['h1_first']===1 && $m['h1_last']===5 && $m['h1c']===2 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1) && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['H','A'] && $m['sc_h']===1 && $m['sc_a']===1) && !(trim($m['away'])==='FC Koln (V)' && $m['h1c']===2 && $m['sc_h']===2 && $m['sc_a']===0 && $m['h1_first']===0 && $m['h1_last']===4 && $m['h1s']===['H','H']) && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===4 && $m['h1c']===3 && $m['h1s']===['H','H','A'] && $m['sc_h']===2 && $m['sc_a']===1 && $m['min_gap']===1 && $m['max_gap']===2) && !($m['h1_first']===0 && $m['sc_h']>$m['sc_a'] && $m['h1_last']>=6 && $m['max_gap']<=3)) || ($m['league']==='15min' && $m['h1_first']===1 && $m['h1_last']>=7 && count($m['h1s'])>0 && $m['h1s'][0]==='A' && $m['h1s'][count($m['h1s'])-1]==='H' && !($m['h1c']===2 && $m['h1_last']===7 && $m['h1_first']===1))) && !($m['league']==='15min' && $m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['H','A'] && $m['sc_h']===1 && $m['sc_a']===1)))],
        [
            'id'=>'P67',
        ...
        ['id'=>'P79', 'label'=>'Structural no-team: 16min + switches=1 + first<=1 + max_gap>=5, kecuali first=1 + h1c=2 + max_gap>=7, tanpa exact team/block team', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['switches']===1 && $m['h1_first']<=1 && $m['max_gap']>=5 && !($m['h1_first']===1 && $m['h1c']===2 && $m['max_gap']>=7)))],
        ...
        ['id'=>'P81', 'label'=>'High-hit no-team union: 16min switches=1 first<=1 max_gap>=5, atau 20min HAA span>=5 min_gap>=2, atau 20min first=1 last=8 switches>=2, atau AAA first>=2 max_gap>=4, atau 20min h1c=2 first=3 span>=5, atau 15min 2-1 max_gap=4 last>=5, atau 20min 1-2 min_gap=3, atau 3H A-start span=6 max_gap=4, atau 16min 1-1 first<=1 span>=4, exclude HAA 1-2 1-8 + AH 1-1 gap7, kecuali kickoff<=6 + first>=1 + last>=9, tanpa exact team/block team', 'data'=>array_values(array_filter($matches, fn($m) => !isP82StructuralMiss($m) && (
        ...
        ) && !(($m['kickoff_hour']??99)<=6 && $m['h1_first']>=1 && $m['h1_last']>=9)))],
        ['id'=>'P82', 'label'=>'No-team union v2 2H: 16min sw=1 first<=1 max_gap>=5; 20min HAA span>=5 min_gap>=2; 20min first=1 last=8 sw>=2; AAA first>=2 max_gap>=4; 20min h1c=2 first=3 span>=5; 15min HT 2-1 max_gap=4 last>=5; 20min HT 1-2 min_gap=3; h1c=3 first scorer A span=6 max_gap=4; 16min HT 1-1 first<=1 span>=4/max_gap>=4; HAA last>=7 span>=6 min_gap>=2; HT 1-2 sw=1 span>=7 min_gap>=2; 20min away lead min_gap=3 sw>=1; 20min span=3 min_gap=0 diff<=2; h1c=3 first A min_gap=2 max_gap=4; 16min last=7 min_gap=1 sw>=1; 16min first H max_gap>=5 sw>=1; exclude HAA 1-2 1-8 + AH 1-1 gap7, kecuali AHA first=3 last=6 h1c=3, kecuali kickoff<=6 + first>=1 + last>=9, tanpa team block/exact team', 'data'=>array_values(array_filter($matches, fn($m) => !isP82StructuralMiss($m) && (
            ($m['league']==='16min' && $m['switches']===1 && $m['h1_first']<=1 && $m['max_gap']>=5)
            || ($m['league']==='20min' && $m['h1s']===['H','A','A'] && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=2)
            || ($m['league']==='20min' && $m['h1_first']===1 && $m['h1_last']===8 && $m['switches']>=2)
            || ($m['h1s']===['A','A','A'] && $m['h1_first']>=2 && $m['max_gap']>=4)
        ...
        ) && !($m['h1_first']===3 && $m['h1_last']===6 && $m['h1c']===3 && $m['h1s']===['A','H','A']) && !(($m['kickoff_hour']??99)<=6 && $m['h1_first']>=1 && $m['h1_last']>=9)))],
        ['id'=>'P83', 'label'=>'No-team union v3 non-0-0 2H: P82 plus 20min first=5 max_gap>=3 sw>=1; HT 2-2 last<=6 min_gap=0 max_gap<=4; first>=1 min_gap=1 max_gap>=4 sw=0; 16min h1c>=3 max_gap<=2 diff<=1; 16min h1c>=3 last<=5 last scorer A; h1c>=2 first=7 last>=8 first scorer H; first=0 last<=2 max_run=2 last scorer A; exclude AH gap7 + HAH 2-1 3-5 + AHA first=3 last=6 h1c=3 + kickoff<=6 last>=9 + first=7 h1c=2 sw=0 max_gap<=1, tanpa HT 0-0, tanpa team block/exact team', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1c']>0 && !isP83StructuralMiss($m) && !($m['h1_first']===3 && $m['h1_last']===6 && $m['h1c']===3 && $m['h1s']===['A','H','A']) && !(($m['kickoff_hour']??99)<=6 && $m['h1_last']>=9) && !($m['h1_first']===7 && $m['h1c']===2 && $m['switches']===0 && $m['max_gap']<=1) && (
            ($m['league']==='16min' && $m['switches']===1 && $m['h1_first']<=1 && $m['max_gap']>=5)
            || ($m['league']==='20min' && $m['h1s']===['H','A','A'] && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=2)
            || ($m['league']==='20min' && $m['h1_first']===1 && $m['h1_last']===8 && $m['switches']>=2)
            || ($m['h1s']===['A','A','A'] && $m['h1_first']>=2 && $m['max_gap']>=4)

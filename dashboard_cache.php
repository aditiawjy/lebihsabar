...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        [
            'id'=>'P24',
            'label'=>'HOME shortlist: Arminia / Osasuna / Arsenal / Leicester / Dortmund / Liverpool / Monaco / Marseille / Atalanta / Spurs / Everton (15min, lm>=4, selisih<=1, HOME cetak>=1, fm>=4, first scorer HOME, Everton khusus lm<=5, bukan single HOME mnt 4/5, Arminia bukan single goal mnt 5-6, bukan Arsenal HA 1-1 mnt 6-7, bukan HAA 1-2 mnt 5-7, kecuali Marseille vs Liverpool)',
            'data'=>array_values(array_filter($matches, fn($m) =>
                $m['league']==='15min' && in_array(trim($m['home']), $p24_teams) && $m['h1c']>=1 && $m['h1_last']>=4 && abs($m['sc_h']-$m['sc_a'])<=1 && $m['sc_h']>=1 && $m['h1_first']>=4 && count($m['h1s'])>0 && $m['h1s'][0]==='H' && (trim($m['home'])!=='Everton (V)' || $m['h1_last']<=5) && (trim($m['home'])!=='Arminia Bielefeld (V)' || !($m['h1c']===1 && $m['h1_first']>=5 && $m['h1_last']<=6)) && !($m['h1c']===1 && $m['h1_first']===4 && $m['h1_last']===4 && $m['h1s']===['H'] && $m['sc_h']===1 && $m['sc_a']===0) && !($m['h1c']===1 && $m['h1_last']===5) && !(trim($m['home'])==='Arsenal (V)' && $m['h1_first']===6 && $m['h1_last']===7 && $m['h1s']===['H','A'] && $m['sc_h']===1 && $m['sc_a']===1) && !($m['h1_first']===5 && $m['h1_last']===7 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !(trim($m['home'])==='Olympique de Marseille (V)' && trim($m['away'])==='Liverpool (V)')
            )),
        ],
        [
            'id'=>'P25',
        ...
        ['id'=>'P42', 'label'=>'20min + first goal >=2 + span >=6 + min_gap>=3, bukan scorer HA, bukan HHA 2-1 mnt 2-9, kecuali max_gap>=7 + h1c=2 + away unggul HT, atau HH 2-0 mnt 2-9', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['h1_first']>=2 && ($m['h1_last']-$m['h1_first'])>=6 && $m['min_gap']>=3 && implode('',$m['h1s'])!=='HA' && !($m['h1_first']===2 && $m['h1_last']===9 && $m['sc_h']===2 && $m['sc_a']===1 && $m['h1s']===['H','H','A']) && !($m['max_gap']>=7 && $m['h1c']===2 && $m['sc_a']>$m['sc_h']) && !($m['h1c']===2 && $m['sc_h']===2 && $m['sc_a']===0 && $m['h1_first']===2 && $m['h1_last']===9 && $m['h1s']===['H','H'])))],
        ...
        [
            'id'=>'P58',
            'label'=>'First goal 1H >=3 + span>=5 + min_gap>=3 + bukan first=4 + span=5 + scorer HH + switches=0, atau 20min + first=2 + span>=5 + min_gap>=3 + (last scorer HOME atau gol 1H>=3) + bukan HH mnt 2-9, atau 16min + first=1 + span>=5 + min_gap>=3 + max_gap>=4, atau 16min + first=0 + span>=5 + min_gap>=3 + selisih HT>=2, bukan 20min HH 2-0 mnt 2-7, bukan 20min HHA 2-1 mnt 2-9, bukan 20min AA 0-2 mnt 4-9, bukan HH 2-0 mnt 0-5/1-7, bukan 16min single AWAY mnt 1/8, bukan 15min HAA 1-2 mnt 2-7, bukan 16min AH 1-1 mnt 3-8, atau 15min h1c=3 + first=2 + span=5 + min_gap=1, atau 15min first=0 + last=6 + span=6 + min_gap=2',
            'data'=>array_values(array_filter($matches, fn($m) =>
                (
        ...
        [
            'id'=>'P61',
            'label'=>'AWAY shortlist: Chelsea / Lille / Juventus / Monaco (15min, last>=5, diff<=1, kecuali Bordeaux vs Lille dan Monaco HA 1-1 mnt 3-5, bukan single goal mnt 5, bukan single AWAY 0-1 mnt 6, bukan AH 1-1 mnt 4-6, bukan AH 1-1 mnt 3-7, bukan AHA 1-2 last=7 min_gap=0, bukan HAA 1-2 mnt 5-7, bukan HHH 3-0 mnt 1-5, bukan AHH 2-1 mnt 1-6, bukan Lille HAA 1-2 mnt 0-6, bukan HAA 1-2 mnt 2-7) atau umum: first<=1 + last>=6 + first&last scorer HOME + diff<=1',
            'data'=>array_values(array_filter($matches, fn($m) =>
                ($m['league']==='15min' && in_array(trim($m['away']), $p61_teams) && $m['h1_last']>=5 && abs($m['sc_h']-$m['sc_a'])<=1 && !(trim($m['home'])==='Girondins de Bordeaux (V)' && trim($m['away'])==='Lille OSC (V)') && !(trim($m['away'])==='AS Monaco (V)' && $m['h1_first']===3 && $m['h1_last']===5 && $m['h1c']===2 && $m['h1s']===['H','A']) && !($m['h1c']===1 && $m['h1_last']===5) && !($m['h1c']===1 && $m['h1_first']===6 && $m['h1_last']===6 && $m['h1s']===['A'] && $m['fh']===0 && $m['fa']===1) && !($m['h1_first']===4 && $m['h1_last']===6 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1) && !($m['h1_first']===3 && $m['h1_last']===7 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1) && !($m['h1_last']===7 && $m['min_gap']===0 && $m['h1s']===['A','H','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===5 && $m['h1_last']===7 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===5 && $m['h1s']===['H','H','H'] && $m['sc_h']===3 && $m['sc_a']===0) && !($m['h1_first']===1 && $m['h1_last']===6 && $m['h1s']===['A','H','H'] && $m['sc_h']===2 && $m['sc_a']===1) && !(trim($m['away'])==='Lille OSC (V)' && $m['h1_first']===0 && $m['h1_last']===6 && $m['h1c']===3 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===2 && $m['h1_last']===7 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2))
        ...
        [
            'id'=>'P65',
            'label'=>'HOME shortlist: Leicester / Napoli / Udinese / Lyon (15min, gol 1H>=1, first<=1, bukan Leicester HH 2-0 mnt 0-5, bukan Leicester AA 0-2 mnt 1-5, bukan Napoli single AWAY mnt 1, bukan HH 2-0 mnt 1-3, bukan AA 0-2 mnt 1-3)',
            'data'=>array_values(array_filter($matches, fn($m) =>
                $m['league']==='15min' && in_array(trim($m['home']), $p65_teams) && $m['h1c']>=1 && $m['h1_first']<=1 && !(trim($m['home'])==='Leicester City (V)' && $m['h1_first']===0 && $m['h1_last']===5 && $m['h1s']===['H','H']) && !(trim($m['home'])==='Leicester City (V)' && $m['h1_first']===1 && $m['h1_last']===5 && $m['sc_h']===0 && $m['sc_a']===2 && $m['h1s']===['A','A']) && !(trim($m['home'])==='Napoli (V)' && $m['h1c']===1 && $m['h1_first']===1 && $m['sc_h']===0 && $m['sc_a']===1 && $m['h1s']===['A']) && !($m['h1_first']===1 && $m['h1_last']===3 && $m['h1s']===['H','H'] && $m['sc_h']===2 && $m['sc_a']===0) && !($m['h1_first']===1 && $m['h1_last']===3 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2)
            )),
        ],
        [
            'id'=>'P66',

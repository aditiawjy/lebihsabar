...
function buildRangeDelta(array $matches, callable $isHit, ?int $rangeStart, ?int $rangeEnd): array {
    if (!$rangeStart || !$rangeEnd || $rangeEnd < $rangeStart) {
        return ['html' => '<span style="color:#484f58">â€”</span>', 'deltaT' => 0, 'deltaH' => 0];
    }
...
function buildDelta(string $id, string $signature, int $total, int $hits, array $oldSnapData): array {
    $html = '<span style="color:#484f58">â€”</span>';
    $deltaT = 0;
    $deltaH = 0;
...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P54', 'label'=>'20min + AWAY unggul HT + last gol 1H mnt 9 + span>=4 + first>=2 + h1c<=4', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['sc_a']>$m['sc_h'] && $m['h1_last']===9 && ($m['h1_last']-$m['h1_first'])>=4 && $m['h1_first']>=2 && $m['h1c']<=4))],
        ...
        ['id'=>'P42', 'label'=>'20min + first goal >=2 + span >=6 + min_gap>=3, bukan scorer HA', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['h1_first']>=2 && ($m['h1_last']-$m['h1_first'])>=6 && $m['min_gap']>=3 && implode('',$m['h1s'])!=='HA'))],
        ...
        ['id'=>'P58', 'label'=>'First goal 1H >=3 + span>=5 + min_gap>=3, atau 20min + first=2 + span>=5 + min_gap>=3 + (last scorer HOME atau gol 1H>=3), atau 16min + first=1 + span>=5 + min_gap>=3 + max_gap>=4, atau 16min + first=0 + span>=5 + min_gap>=3 + selisih HT>=2', 'data'=>array_values(array_filter($matches, fn($m) => ($m['h1_first']>=3 && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=3) || ($m['league']==='20min' && $m['h1_first']===2 && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=3 && ((count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H') || $m['h1c']>=3)) || ($m['league']==='16min' && $m['h1_first']===1 && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=3 && $m['max_gap']>=4) || ($m['league']==='16min' && $m['h1_first']===0 && ($m['h1_last']-$m['h1_first'])>=5 && $m['min_gap']>=3 && abs($m['sc_h']-$m['sc_a'])>=2)))],
        [
            'id' => 'P59',

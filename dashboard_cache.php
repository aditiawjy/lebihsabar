...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P6',  'label'=>'Seri 1-1 + gol penyama mnt 7\' + span>=5 + first!=1 + home!=Manchester City/Atletico + first scorer AWAY', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1c']==2 && $m['sc_h']==1 && $m['sc_a']==1 && $m['h1_last']==7 && ($m['h1_last']-$m['h1_first'])>=5 && $m['h1_first']!=1 && !in_array(trim($m['home']), ['Manchester City (V)', 'Atletico de Madrid (V)'], true) && count($m['h1s'])==2 && $m['h1s'][0]==='A'))],
        ...
        ['id'=>'P13', 'label'=>'First 2\' + last 7\' + selisih <=2 + min_gap>=3 + switches>=1, kecuali Man City vs Liverpool', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1c']>=2 && $m['h1_first']===2 && $m['h1_last']===7 && abs($m['sc_h']-$m['sc_a'])<=2 && $m['min_gap']>=3 && $m['switches']>=1 && !(trim($m['home'])==='Manchester City (V)' && trim($m['away'])==='Liverpool (V)')))],
        ...
        ['id'=>'P17', 'label'=>'First 1H mnt 1-2 + last mnt 7 + min_gap>=2 + switches>=1 + first scorer AWAY + (20min atau 15min first=1) + jam>=06:00', 'data'=>array_values(array_filter($matches, function($m) { $dt = DateTime::createFromFormat('d/m/Y H:i', $m['datetime'] ?? ''); $hour = $dt ? (int)$dt->format('H') : -1; return $m['h1c']>=2 && $m['h1_first']>=1 && $m['h1_first']<=2 && $m['h1_last']==7 && $m['max_gap']>=2 && $m['min_gap']>=2 && $m['switches']>=1 && count($m['h1s'])>0 && $m['h1s'][0]==='A' && ($m['league']==='20min' || ($m['league']==='15min' && $m['h1_first']===1)) && $hour >= 6; }))],
        ['id'=>'P19', 'label'=>'Last gol 1H mnt 3, last HOME, 20min, atau mnt 4 jika AWAY unggul HT / switches>=2 + first goal<=1 + switches>=1', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H' && $m['h1c']>=2 && ($m['h1_last']===3 || ($m['h1_last']===4 && ($m['sc_a']>$m['sc_h'] || $m['switches']>=2))) && $m['h1_first']<=1 && $m['switches']>=1))],
        ['id'=>'P53', 'label'=>'20min + last gol 1H mnt 3 + last scorer HOME + min_gap>=1 + gol 1H<=2 + (first=0 atau away HT=0)', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['h1_last']===3 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H' && $m['min_gap']>=1 && $m['h1c']<=2 && ($m['h1_first']===0 || $m['sc_a']===0)))],
        ...
        ['id'=>'P24', 'label'=>'HOME shortlist: Arminia / Osasuna / Arsenal / Leicester / Dortmund / Liverpool / Monaco / Marseille / Atalanta / Spurs (15min, lm>=4, selisih<=1, HOME cetak>=1, fm>=4 atau >=6, first scorer HOME)', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='15min' && in_array(trim($m['home']), $p24_teams) && $m['h1c']>=1 && $m['h1_last']>=4 && abs($m['sc_h']-$m['sc_a'])<=1 && $m['sc_h']>=1 && $m['h1_first']>=4 && $m['h1_first']!==5 && count($m['h1s'])>0 && $m['h1s'][0]==='H'))],
        ['id'=>'P25', 'label'=>'AWAY: Real Sociedad / France / Netherlands / Ukraine (lm>=2, selisih<=1, span>=3, min_gap>=2, last scorer AWAY)', 'data'=>array_values(array_filter($matches, fn($m) => in_array(trim($m['away']), $p25_teams) && $m['h1_last']>=2 && abs($m['sc_h']-$m['sc_a'])<=1 && ($m['h1_last']-$m['h1_first'])>=3 && $m['min_gap']>=2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A'))],
        ...
        ['id'=>'P54', 'label'=>'20min + AWAY unggul HT + last gol 1H mnt 9 + span>=4 + first>=2', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['sc_a']>$m['sc_h'] && $m['h1_last']===9 && ($m['h1_last']-$m['h1_first'])>=4 && $m['h1_first']>=2))],
        ...
        ['id'=>'P35', 'label'=>'AWAY shortlist: Mexico / Belgium / Germany / AS Monaco / Wales / Portugal / Osasuna / Austria / Poland / Croatia + gol 1H>=2 + first>=3 + max_run<=2 + last gol 1H>=6 + min_gap>=2', 'data'=>array_values(array_filter($matches, fn($m) => in_array(trim($m['away']), $p35_teams) && $m['h1c']>=2 && $m['h1_first']>=3 && $m['max_run']<=2 && $m['h1_last']>=6 && $m['min_gap']>=2))],
        ...
        ['id'=>'P42', 'label'=>'20min + first goal >=2 + span >=6 + min_gap>=3', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['h1_first']>=2 && ($m['h1_last']-$m['h1_first'])>=6 && $m['min_gap']>=3))],
        ...
        ['id'=>'P44', 'label'=>'Selisih HT >=2 + first goal >=2 + switches>=1 + max_run<=2, kecuali Inter vs FC Koln', 'data'=>array_values(array_filter($matches, fn($m) => abs($m['sc_h']-$m['sc_a'])>=2 && $m['h1_first']>=2 && $m['switches']>=1 && $m['max_run']<=2 && !(trim($m['home'])==='Inter Milan (V)' && trim($m['away'])==='FC Koln (V)')))],
        ...
        ['id'=>'P57', 'label'=>'First goal 1H mnt 0 + last gol 1H mnt 6 + first scorer AWAY + (15min atau gol 1H>=3) + (switches>=2 atau max_gap>=4)', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1_first']===0 && $m['h1_last']===6 && count($m['h1s'])>0 && $m['h1s'][0]==='A' && ($m['league']==='15min' || $m['h1c']>=3) && ($m['switches']>=2 || $m['max_gap']>=4)))],
        ...
        ['id'=>'P59', 'label'=>'Last gol 1H mnt 9 + switches>=2 + last scorer AWAY + (first scorer AWAY atau max_gap>=6) + (h1c>=4 atau max_gap<6)', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1_last']===9 && $m['switches']>=2 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A' && ($m['h1s'][0]==='A' || $m['max_gap']>=6) && ($m['h1c']>=4 || $m['max_gap']<6)))],
        ['id'=>'P60', 'label'=>'20min + first goal 1H mnt 3 + HT seri + last gol 1H>=5 + last scorer HOME', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['h1_first']===3 && $m['sc_h']===$m['sc_a'] && $m['h1_last']>=5 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H'))],
        ['id'=>'P61', 'label'=>'AWAY shortlist: Chelsea / Lille / Juventus / Monaco (15min, last>=6, diff<=1)', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='15min' && in_array(trim($m['away']), $p61_teams) && $m['h1_last']>=6 && abs($m['sc_h']-$m['sc_a'])<=1))],
        ['id'=>'P62', 'label'=>'HOME shortlist: Getafe / Osasuna / FC Koln / Lazio / Leicester / Napoli / Sevilla / Udinese (15min, first<=1, last>=4, first=0 atau non-FC Koln, switches>=1 atau gol 1H<=3 atau last>=7)', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='15min' && in_array(trim($m['home']), $p62_teams) && $m['h1_first']<=1 && $m['h1_last']>=4 && ($m['h1_first']===0 || trim($m['home'])!=='FC Koln (V)') && ($m['switches']>=1 || $m['h1c']<=3 || $m['h1_last']>=7)))],
        ...
        ['id'=>'P64', 'label'=>'AWAY shortlist: Liverpool / Napoli / Bayern / FC Koln / FSV Mainz / Lille (15min, first<=1, last>=4, Napoli khusus max_run<=2)', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='15min' && in_array(trim($m['away']), $p64_teams) && $m['h1_first']<=1 && $m['h1_last']>=4 && (trim($m['away'])!=='Napoli (V)' || $m['max_run']<=2)))],
        ...
        ['id'=>'P66', 'label'=>'AWAY shortlist: Mainz / Getafe / Lille / Liverpool / Lyon / Juventus / Dortmund / Napoli / Bayern / FC Koln (15min, first<=1, last>=5, Napoli khusus max_run<=2)', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='15min' && in_array(trim($m['away']), $p66_teams) && $m['h1_first']<=1 && $m['h1_last']>=5 && (trim($m['away'])!=='Napoli (V)' || $m['max_run']<=2)))],
        ['id'=>'P67', 'label'=>'HOME shortlist: Argentina / Denmark / Germany / Russia (20min, first<=1, last>=5)', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && in_array(trim($m['home']), $p67_teams) && $m['h1_first']<=1 && $m['h1_last']>=5))],
        ...
        ['id'=>'P72', 'label'=>'20min + gol 1H=2 + span>=5 + first>=3', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['h1c']===2 && ($m['h1_last']-$m['h1_first'])>=5 && $m['h1_first']>=3))],
        ['id'=>'P73', 'label'=>'First gol 1H mnt 3 + last mnt 8 + gol 1H=2', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1_first']===3 && $m['h1_last']===8 && $m['h1c']===2))],
        ['id'=>'P74', 'label'=>'First gol 1H mnt 4 + last mnt 9 + gol 1H=2', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1_first']===4 && $m['h1_last']===9 && $m['h1c']===2))],
        ['id'=>'P75', 'label'=>'16min + gol 1H=2 + span>=4 + HOME unggul HT', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['h1c']===2 && ($m['h1_last']-$m['h1_first'])>=4 && $m['sc_h']>$m['sc_a']))],
        ['id'=>'P76', 'label'=>'Gol 1H=3 + first mnt 2 + last mnt 9', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1c']===3 && $m['h1_first']===2 && $m['h1_last']===9))],
        ['id'=>'P77', 'label'=>'HT seri 1-1 + gol 1H=2 + first=0 + last=6', 'data'=>array_values(array_filter($matches, fn($m) => $m['sc_h']===1 && $m['sc_a']===1 && $m['h1c']===2 && $m['h1_first']===0 && $m['h1_last']===6))],
    ];
}

function computeNextPatterns(array $matches): array {
    return [
        ['id'=>'NG6','label'=>'20min + seri 1-1 + scorer AH + last gol 1H mnt 7 + span>=5 + first!=1','next'=>'AWAY','data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['sc_h']==1 && $m['sc_a']==1 && $m['h1_last']==7 && ($m['h1_last']-$m['h1_first'])>=5 && $m['h1s']==['A','H'] && $m['h1_first']!=1))],
        ['id'=>'NG7','label'=>'Gol 1H >=3 + max_gap>=5 + AWAY unggul HT tepat 2 + last gol 1H>=8','next'=>'AWAY','data'=>array_values(array_filter($matches, fn($m) => $m['h1c']>=3 && $m['max_gap']>=5 && $m['sc_a']-$m['sc_h']===2 && $m['h1_last']>=8))],
...
function computeLatePatterns(array $matches): array {
    ...
    $latePatterns = [
        ...
        [
            'id' => 'LG4',
            'label' => 'AWAY shortlist: Norway / Uruguay / Algeria / Nigeria / Romania (20min, away lead HT, last gol 1H 9, first!=1)',
            'data' => array_values(array_filter($matches, fn($m) => $m['league'] === '20min' && in_array(trim($m['away']), $lg4_teams, true) && $m['sc_a'] > $m['sc_h'] && $m['h1_last'] === 9 && $m['h1_first'] !== 1)),
        ],
        [
            'id' => 'LG5',
        ...
        [
            'id' => 'LG6',
            'label' => 'AWAY shortlist: Indonesia / Algeria / Slovakia / Slovenia (20min, first<=1, last gol 1H >=8, span>=8)',
            'data' => array_values(array_filter($matches, fn($m) => $m['league'] === '20min' && in_array(trim($m['away']), $lg6_teams, true) && $m['h1_first'] <= 1 && $m['h1_last'] >= 8 && ($m['h1_last'] - $m['h1_first']) >= 8)),
        ],
        [
            'id' => 'LG7',
            'label' => 'AWAY shortlist: Nigeria / Qatar / Slovenia (away lead HT, last gol 1H >=6, gol 1H<=3)',
            'data' => array_values(array_filter($matches, fn($m) => in_array(trim($m['away']), $lg7_teams, true) && $m['sc_a'] > $m['sc_h'] && $m['h1_last'] >= 6 && $m['h1c'] <= 3)),
        ],
        [
            'id' => 'LG8',
            'label' => 'AWAY shortlist: Norway / Nigeria / Poland / Slovenia (20min, away lead HT, last gol 1H >=5, gol 1H>=2, first>=2 atau span>=5)',
            'data' => array_values(array_filter($matches, fn($m) => $m['league'] === '20min' && in_array(trim($m['away']), $lg8_teams, true) && $m['sc_a'] > $m['sc_h'] && $m['h1_last'] >= 5 && $m['h1c'] >= 2 && ($m['h1_first'] >= 2 || ($m['h1_last'] - $m['h1_first']) >= 5))),
        ],
    ];

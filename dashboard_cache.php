...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P29', 'label'=>'Balas >=2x + last mnt >=6 + min_gap>=3 + span>=6 + selisih<=1, gol>=3', 'data'=>array_values(array_filter($matches, fn($m) => $m['switches'] >= 2 && $m['h1_last'] >= 6 && $m['min_gap'] >= 3 && ($m['h1_last']-$m['h1_first']) >= 6 && abs($m['sc_h']-$m['sc_a']) <= 1 && $m['h1c'] >= 3))],
        ...
        ['id'=>'P44', 'label'=>'Selisih HT >=2 + first goal >=2 + switches>=1', 'data'=>array_values(array_filter($matches, fn($m) => abs($m['sc_h']-$m['sc_a'])>=2 && $m['h1_first']>=2 && $m['switches']>=1))],

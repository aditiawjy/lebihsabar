...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P3',  'label'=>'AH gap >=4 mnt + lm>=4 + fm>=2, 15min/16min', 'data'=>array_values(array_filter($matches, fn($m) => in_array($m['league'],['15min','16min']) && $m['h1c']==2 && $m['sc_h']==1 && $m['sc_a']==1 && $m['h1s']==['A','H'] && ($m['h1'][1]['min']-$m['h1'][0]['min']) >= 4 && $m['h1_last']>=4 && $m['h1_first']>=2))],
        ...
        ['id'=>'P13', 'label'=>'First 0/2\' + last 7\' + selisih <=2 + min_gap>=3 + switches>=1', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1c']>=2 && in_array($m['h1_first'], [0, 2], true) && $m['h1_last']===7 && abs($m['sc_h']-$m['sc_a'])<=2 && $m['min_gap']>=3 && $m['switches']>=1))],
        ...
        ['id'=>'P16', 'label'=>'Last gol 1H mnt 7 + span >=3 + switches>=1 + first!=1, 16min', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['h1_last']==7 && ($m['h1_last']-$m['h1_first'])>=3 && $m['switches']>=1 && $m['h1_first']!=1))],
        ...
        ['id'=>'P21', 'label'=>'Last gol 1H mnt 5, last AWAY, 15min, max_gap>=2 + min_gap>=1 + sw>=1 + max_run<=2 (n1h>=3 atau AWAY unggul)', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='15min' && $m['h1_last']===5 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A' && $m['max_gap']>=2 && $m['min_gap']>=1 && ($m['h1c']>=3 || $m['sc_a']>$m['sc_h']) && $m['switches']>=1 && $m['max_run']<=2))],
        ...
        ['id'=>'P26', 'label'=>'HT total ganjil (1,3,5...), 16min, last mnt >=6, gol 1H >=2, away unggul', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && ($m['sc_h']+$m['sc_a'])%2===1 && $m['h1_last']>=6 && $m['h1c']>=2 && $m['sc_a']>$m['sc_h']))],
        ...
        ['id'=>'P37', 'label'=>'16min + first & last scorer AWAY + first<=1 + gol 1H>=2 + tanpa balas HOME', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['h1c']>=2 && $m['h1_first']<=1 && count($m['h1s'])>0 && $m['h1s'][0]==='A' && $m['h1s'][count($m['h1s'])-1]==='A' && $m['switches']===0))],
        ['id'=>'P39', 'label'=>'Gol 1H >=3 + span >=7 mnt + selisih <=3 + fm>=1 + min_gap>=3, 20min', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && $m['h1c']>=3 && ($m['h1_last']-$m['h1_first'])>=7 && abs($m['sc_h']-$m['sc_a'])<=3 && $m['min_gap']>=3 && $m['h1_first']>=1))],
        ['id'=>'P40', 'label'=>'16min + selisih HT >=2 + first goal <=1', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && abs($m['sc_h']-$m['sc_a'])>=2 && $m['h1_first']<=1))],
        ['id'=>'P41', 'label'=>'Selisih HT >=2 + first goal >=2 + span >=6', 'data'=>array_values(array_filter($matches, fn($m) => abs($m['sc_h']-$m['sc_a'])>=2 && $m['h1_first']>=2 && ($m['h1_last']-$m['h1_first'])>=6))],
        ...
        ['id'=>'P43', 'label'=>'Away unggul HT + span >=6 + last scorer HOME', 'data'=>array_values(array_filter($matches, fn($m) => $m['sc_a']>$m['sc_h'] && ($m['h1_last']-$m['h1_first'])>=6 && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H'))],
        ['id'=>'P44', 'label'=>'20min + selisih HT >=2 + first goal >=2 + switches>=1 + last<=9', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && abs($m['sc_h']-$m['sc_a'])>=2 && $m['h1_first']>=2 && $m['switches']>=1 && $m['h1_last']<=9))],
        ['id'=>'P45', 'label'=>'16min + first goal !=1 + span >=6', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['h1_first']!=1 && ($m['h1_last']-$m['h1_first'])>=6))],
        ['id'=>'P46', 'label'=>'16min + span >=6 + min_gap>=2', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && ($m['h1_last']-$m['h1_first'])>=6 && $m['min_gap']>=2))],
        ...
        ['id'=>'P50', 'label'=>'16min + away unggul HT + span >=6', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['sc_a']>$m['sc_h'] && ($m['h1_last']-$m['h1_first'])>=6))],
        ['id'=>'P51', 'label'=>'16min + switches>=2', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['switches']>=2))],
        ['id'=>'P52', 'label'=>'16min + span >=6 + min_gap>=3', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && ($m['h1_last']-$m['h1_first'])>=6 && $m['min_gap']>=3))],
    ];
}
...
function computeNextPatterns(array $matches): array {
    return [
        ...
        ['id'=>'NG2','label'=>'HOME unggul 1 gol HT + gol terakhir mnt 3, 16min','next'=>'HOME','data'=>array_values(array_filter($matches, fn($m) => $m['league']==='16min' && $m['h1c']>=1 && ($m['sc_h']-$m['sc_a'])===1 && $m['h1_last']==3))],
        ['id'=>'NG5','label'=>'Seri 1-1 + scorer HA + gol terakhir mnt 7 + fm>=2','next'=>'AWAY','data'=>array_values(array_filter($matches, fn($m) => $m['sc_h']==1 && $m['sc_a']==1 && $m['h1c']==2 && count($m['h1s'])>=2 && $m['h1s'][0]==='H' && $m['h1s'][count($m['h1s'])-1]==='A' && $m['h1_last']==7 && $m['h1_first']>=2))],

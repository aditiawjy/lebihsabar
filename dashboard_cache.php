...
function computePatterns(array $matches): array {
    ...
    return [
        ...
        ['id'=>'P7',  'label'=>'Seri 1-1 + gap >= 5 mnt + first goal >=3, bukan 16min AH 1-1 mnt 3-8', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1c']==2 && $m['sc_h']==1 && $m['sc_a']==1 && $m['max_gap']>=5 && $m['h1_first']>=3 && !($m['league']==='16min' && $m['h1_first']===3 && $m['h1_last']===8 && $m['h1s']===['A','H'])))],
        ...
        ['id'=>'P14', 'label'=>'Seri + gap >= 4 mnt + span >= 5 mnt + first goal >=3 + min_gap>=2, bukan 16min AH 1-1 mnt 3-8', 'data'=>array_values(array_filter($matches, fn($m) => $m['h1c']>=2 && $m['sc_h']===$m['sc_a'] && $m['sc_h']>0 && $m['max_gap']>=4 && ($m['h1_last']-$m['h1_first'])>=5 && $m['h1_first']>=3 && $m['min_gap']>=2 && !($m['league']==='16min' && $m['h1_first']===3 && $m['h1_last']===8 && $m['h1s']===['A','H'])))],
        ...
        ['id'=>'P19', 'label'=>'20min + last scorer HOME + first<=1 + switches>=1, lalu: last=3 (first=0 atau gol 1H>=3) atau last=4 (AWAY unggul HT / switches>=2), bukan HHHAHH 5-1 mnt 1-3', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='H' && $m['h1c']>=2 && ((($m['h1_last']===3) && ($m['h1_first']===0 || $m['h1c']>=3)) || ($m['h1_last']===4 && ($m['sc_a']>$m['sc_h'] || $m['switches']>=2))) && $m['h1_first']<=1 && $m['switches']>=1 && !($m['h1_first']===1 && $m['h1_last']===3 && $m['h1s']===['H','H','H','A','H','H'] && $m['sc_h']===5 && $m['sc_a']===1)))],
        ...
        ['id'=>'P28', 'label'=>'Croatia atau France main + last mnt >=3 + span >=3 + switches>=1 + (target team away atau first>=2), bukan 20min HAH 2-1 first=2 last>=9, bukan 16min HAA 1-2 mnt 5-8', 'data'=>array_values(array_filter($matches, fn($m) => (in_array(trim($m['home']), $p28_teams) || in_array(trim($m['away']), $p28_teams)) && $m['h1_last']>=3 && ($m['h1_last']-$m['h1_first'])>=3 && $m['switches']>=1 && (in_array(trim($m['away']), $p28_teams) || $m['h1_first']>=2) && !($m['league']==='20min' && $m['h1_first']===2 && $m['h1_last']>=9 && $m['sc_h']===2 && $m['sc_a']===1 && $m['h1s']===['H','A','H']) && !($m['league']==='16min' && $m['h1_first']===5 && $m['h1_last']===8 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2)))],
        ...
        [
            'id'=>'P54',
            'label'=>'20min + AWAY unggul HT + last gol 1H mnt 9 + span>=4 + first>=2 + h1c<=4, bukan first=2 + scorer AAH, bukan AA 0-2 mnt 2-9/4-9, bukan HAA 1-2 mnt 5-9',
            'data'=>array_values(array_filter($matches, fn($m) =>
                $m['league']==='20min' && $m['sc_a']>$m['sc_h'] && $m['h1_last']===9 && ($m['h1_last']-$m['h1_first'])>=4 && $m['h1_first']>=2 && $m['h1c']<=4 && !($m['h1_first']===2 && $m['h1s']===['A','A','H']) && !($m['h1_first']===2 && $m['h1_last']===9 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===4 && $m['h1_last']===9 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===5 && $m['h1_last']===9 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2)
            )),
        ],
        [
            'id'=>'P33',
        ...
        ['id'=>'P34', 'label'=>'First AWAY + last HOME + span >=6 + gol 1H >=4, 15min, bukan AAHH 2-2 mnt 0-7 min_gap=2, bukan AHAHH 3-2 mnt 0-7', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='15min' && count($m['h1s'])>0 && $m['h1s'][0]==='A' && $m['h1s'][count($m['h1s'])-1]==='H' && ($m['h1_last']-$m['h1_first'])>=6 && $m['h1c']>=4 && !($m['h1_first']===0 && $m['h1_last']===7 && $m['h1s']===['A','A','H','H'] && $m['sc_h']===2 && $m['sc_a']===2 && $m['min_gap']===2) && !($m['h1_first']===0 && $m['h1_last']===7 && $m['h1s']===['A','H','A','H','H'] && $m['sc_h']===3 && $m['sc_a']===2)))],
        [
            'id'=>'P35',
            'label'=>'AWAY shortlist: Mexico / Belgium / Germany / AS Monaco / Wales / Portugal / Osasuna / Austria / Poland / Croatia / Algeria + gol 1H>=2 + first>=3 + max_run<=2 + last gol 1H>=5 + min_gap>=2 + bukan HH mnt 3&5 + bukan 16min HH 2-0 first>=5 last=7 + bukan 20min AA 0-2 mnt 4-9 + bukan HA 1-1 mnt 3-5 + bukan HAA mnt 3-7 + bukan 20min AH 1-1 mnt 4-6, atau 16min shortlist first=1 + h1c>=3, atau shortlist first=0 + h1c>=3 + last>=6 + last scorer AWAY + bukan 20min AHA 1-2 mnt 0-7, atau Portugal single AWAY bukan mnt 0, atau h1c=3 scorer HHA min_gap=1',
            'data'=>array_values(array_filter($matches, fn($m) =>
                (in_array(trim($m['away']), $p35_teams) && $m['h1c']>=2 && $m['h1_first']>=3 && $m['max_run']<=2 && $m['h1_last']>=5 && $m['min_gap']>=2 && !(count($m['h1s'])===2 && $m['h1s'][0]==='H' && $m['h1s'][1]==='H' && $m['h1_first']===3 && $m['h1_last']===5) && !($m['league']==='16min' && $m['h1c']===2 && $m['sc_h']===2 && $m['sc_a']===0 && $m['h1s']===['H','H'] && $m['h1_first']>=5 && $m['h1_last']===7) && !($m['league']==='20min' && $m['h1_first']===4 && $m['h1_last']===9 && $m['h1c']===2 && $m['sc_h']===0 && $m['sc_a']===2 && $m['h1s']===['A','A']) && !($m['h1_first']===3 && $m['h1_last']===5 && $m['h1c']===2 && $m['sc_h']===1 && $m['sc_a']===1 && $m['h1s']===['H','A']) && !($m['h1_first']===3 && $m['h1_last']===7 && $m['h1s']===['H','A','A']) && !($m['league']==='20min' && $m['h1_first']===4 && $m['h1_last']===6 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1))
        ...
        ['id'=>'P40', 'label'=>'16min + selisih HT tepat 2 + first goal <=1 + span>=5 + (min_gap>=1 atau max_gap>=4), bukan HH 2-0 mnt 0-5/1-7, bukan last AWAY + home lead>=2 + max_run>=3 + max_gap<=3', 'data'=>array_values(array_filter($matches, fn($m) =>
            $m['league']==='16min' && abs($m['sc_h']-$m['sc_a'])===2 && $m['h1_first']<=1 && ($m['h1_last']-$m['h1_first'])>=5 && ($m['min_gap']>=1 || $m['max_gap']>=4) && !(($m['h1_first']===0 && $m['h1_last']===5 && $m['h1s']===['H','H'] && $m['sc_h']===2 && $m['sc_a']===0) || ($m['h1_first']===1 && $m['h1_last']===7 && $m['h1s']===['H','H'] && $m['sc_h']===2 && $m['sc_a']===0)) && !(count($m['h1s'])>0 && $m['h1s'][count($m['h1s'])-1]==='A' && ($m['sc_h']-$m['sc_a'])>=2 && $m['max_run']>=3 && $m['max_gap']<=3)
        ))],
        ...
        [
            'id'=>'P57',
            'label'=>'First goal 1H mnt 0 + last gol 1H mnt 6 + first scorer AWAY + (15min atau 16min atau gol 1H>=3) + (switches>=2 atau max_gap>=4) + bukan scorer AHH/AAA + bukan 15min AA 0-2 mnt 0-6',
            'data'=>array_values(array_filter($matches, fn($m) =>
        ...
        [
            'id'=>'P58',
            'label'=>'First goal 1H >=3 + span>=5 + min_gap>=3 + bukan first=4 + span=5 + scorer HH + switches=0, atau 20min + first=2 + span>=5 + min_gap>=3 + (last scorer HOME atau gol 1H>=3) + bukan HH mnt 2-9, atau 16min + first=1 + span>=5 + min_gap>=3 + max_gap>=4, atau 16min + first=0 + span>=5 + min_gap>=3 + selisih HT>=2, bukan 20min AA 0-2 mnt 4-9, bukan HH 2-0 mnt 0-5/1-7, bukan 16min single AWAY mnt 1/8, bukan 15min HAA 1-2 mnt 2-7, bukan 16min AH 1-1 mnt 3-8, atau 15min h1c=3 + first=2 + span=5 + min_gap=1, atau 15min first=0 + last=6 + span=6 + min_gap=2',
            'data'=>array_values(array_filter($matches, fn($m) =>
                (
        ...
        [
            'id'=>'P61',
            'label'=>'AWAY shortlist: Chelsea / Lille / Juventus / Monaco (15min, last>=5, diff<=1, kecuali Bordeaux vs Lille dan Monaco HA 1-1 mnt 3-5, bukan single goal mnt 5, bukan single AWAY 0-1 mnt 6, bukan AH 1-1 mnt 4-6, bukan AH 1-1 mnt 3-7, bukan HAA 1-2 mnt 5-7, bukan HHH 3-0 mnt 1-5, bukan AHH 2-1 mnt 1-6, bukan Lille HAA 1-2 mnt 0-6, bukan HAA 1-2 mnt 2-7) atau umum: first<=1 + last>=6 + first&last scorer HOME + diff<=1',
            'data'=>array_values(array_filter($matches, fn($m) =>
                ($m['league']==='15min' && in_array(trim($m['away']), $p61_teams) && $m['h1_last']>=5 && abs($m['sc_h']-$m['sc_a'])<=1 && !(trim($m['home'])==='Girondins de Bordeaux (V)' && trim($m['away'])==='Lille OSC (V)') && !(trim($m['away'])==='AS Monaco (V)' && $m['h1_first']===3 && $m['h1_last']===5 && $m['h1c']===2 && $m['h1s']===['H','A']) && !($m['h1c']===1 && $m['h1_last']===5) && !($m['h1c']===1 && $m['h1_first']===6 && $m['h1_last']===6 && $m['h1s']===['A'] && $m['fh']===0 && $m['fa']===1) && !($m['h1_first']===4 && $m['h1_last']===6 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1) && !($m['h1_first']===3 && $m['h1_last']===7 && $m['h1s']===['A','H'] && $m['sc_h']===1 && $m['sc_a']===1) && !($m['h1_first']===5 && $m['h1_last']===7 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===5 && $m['h1s']===['H','H','H'] && $m['sc_h']===3 && $m['sc_a']===0) && !($m['h1_first']===1 && $m['h1_last']===6 && $m['h1s']===['A','H','H'] && $m['sc_h']===2 && $m['sc_a']===1) && !(trim($m['away'])==='Lille OSC (V)' && $m['h1_first']===0 && $m['h1_last']===6 && $m['h1c']===3 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===2 && $m['h1_last']===7 && $m['h1s']===['H','A','A'] && $m['sc_h']===1 && $m['sc_a']===2))
                || ($m['league']==='15min' && $m['h1_first']<=1 && $m['h1_last']>=6 && abs($m['sc_h']-$m['sc_a'])<=1 && count($m['h1s'])>0 && $m['h1s'][0]==='H' && $m['h1s'][count($m['h1s'])-1]==='H')
            )),
        ],
        [
            'id'=>'P62',
            'label'=>'HOME shortlist: Getafe / Osasuna / FC Koln / Lazio / Leicester / Napoli / Sevilla / Udinese (bukan first=1 + last=4 + scorer AAH, bukan Leicester HH 2-0 mnt 0-5, bukan Leicester AA 0-2 mnt 1-5, bukan AA 0-2 mnt 0-5), atau 15min umum: first<=1 + last>=6 + switches>=2 + diff<=2 + last scorer HOME, bukan scorer HHAH, atau 15min first=3 + last=7 + diff=2 + last HOME, atau 15min HH 2-0 mnt 2-4, atau 16min: first<=1 + last>=7 + diff<=2 + last scorer HOME, atau 20min: first<=1 + last>=3 + switches>=2 + HT seri + last scorer HOME, atau 20min HH 2-0 mnt 1-4, kecuali Getafe away dengan scorer AHA, bukan 15min h1c=5 + HT 3-2 + first=0 + last>=7 + min_gap=0, bukan 16min HH 2-0 mnt 1-7, bukan 16min AHHAH 3-2 mnt 0-8, bukan 20min HAAH 2-2 mnt 0-5',
            'data'=>array_values(array_filter($matches, fn($m) =>
        ...
        [
            'id'=>'P67',
            'label'=>'HOME shortlist: Argentina / Denmark / Germany / Russia / Korea Republic / Croatia / Brazil / Nigeria / Serbia / USA / South Africa / Portugal / Oman / New Zealand / Philippines / Iran / Iraq (20min, first<=1, last>=5, bukan HH first=1 last=7 h1c=2, bukan HH 2-0 first=1 last=6, bukan AAHAA first=1 last=10, bukan AHA mnt 0-4-8, bukan AAH 1-2 mnt 0-5, bukan AHA 1-2 mnt 0-7, bukan AA 0-2 mnt 0-6, bukan AA 0-2 mnt 1-5, bukan HAAH 2-2 mnt 0-5)',
            'data'=>array_values(array_filter($matches, fn($m) =>
                $m['league']==='20min' && in_array(trim($m['home']), $p67_teams) && $m['h1_first']<=1 && $m['h1_last']>=5 && !($m['h1_first']===1 && $m['h1_last']===7 && $m['h1c']===2 && $m['h1s']===['H','H']) && !($m['h1_first']===1 && $m['h1_last']===6 && $m['h1c']===2 && $m['h1s']===['H','H'] && $m['sc_h']===2 && $m['sc_a']===0) && !($m['h1_first']===1 && $m['h1_last']===10 && $m['h1s']===['A','A','H','A','A']) && !($m['h1_first']===0 && $m['h1_last']===8 && $m['h1c']===3 && $m['h1s']===['A','H','A']) && !($m['h1_first']===0 && $m['h1_last']===5 && $m['h1c']===3 && $m['h1s']===['A','A','H'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===0 && $m['h1_last']===7 && $m['h1c']===3 && $m['h1s']===['A','H','A'] && $m['sc_h']===1 && $m['sc_a']===2) && !($m['h1_first']===0 && $m['h1_last']===6 && $m['h1c']===2 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===1 && $m['h1_last']===5 && $m['h1c']===2 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2) && !($m['h1_first']===0 && $m['h1_last']===5 && $m['h1s']===['H','A','A','H'] && $m['sc_h']===2 && $m['sc_a']===2)
            )),
        ],
        ...
        ['id'=>'P71', 'label'=>'AWAY single: Germany (20min, away lead HT, last>=6, bukan AA 0-2 mnt 4-9)', 'data'=>array_values(array_filter($matches, fn($m) => $m['league']==='20min' && trim($m['away'])==='Germany (V)' && $m['sc_a']>$m['sc_h'] && $m['h1_last']>=6 && !($m['h1_first']===4 && $m['h1_last']===9 && $m['h1s']===['A','A'] && $m['sc_h']===0 && $m['sc_a']===2)))],
    ];
}
...
function computeLatePatterns(array $matches): array {
    ...
    $latePatterns = [
        ...
        [
            'id' => 'LG4',
            'label' => 'AWAY shortlist: Norway / Uruguay / Algeria / Nigeria / Romania (20min, away lead HT, last gol 1H 9, bukan h1c=3 first=1, bukan single AWAY mnt 9)',
            'data' => array_values(array_filter($matches, fn($m) =>
        ...
        [
            'id' => 'LG6',
            'label' => 'AWAY shortlist: Indonesia / Algeria / Slovakia / Slovenia (20min, first<=1, last gol 1H >=8, first=0 atau away unggul HT, bukan h1c=2 scorer AA, bukan AHA mnt 0-4-8, bukan AAAA 0-4 mnt 0-9), atau Argentina (first=0, last>=7, h1c>=3, selisih<=1)',
            'data' => array_values(array_filter($matches, fn($m) =>
        ...
        [
            'id' => 'LG7',
            'label' => 'AWAY shortlist: Nigeria / Qatar / Slovenia (away lead HT, gol 1H<=3, dan gol 1H>=2 atau last gol 1H>=7, bukan h1c=2 first=0, bukan single AWAY mnt 8, bukan single goal 2H mnt <7, bukan AA 0-2 mnt 2-9)',
            'data' => array_values(array_filter($matches, fn($m) =>
                in_array(trim($m['away']), $lg7_teams, true) && $m['sc_a'] > $m['sc_h'] && $m['h1_last'] >= 6 && $m['h1c'] <= 3 && ($m['h1c'] >= 2 || $m['h1_last'] >= 7) && !($m['h1c'] === 2 && $m['h1_first'] === 0) && !($m['h1c'] === 1 && $m['h1_first'] === 8 && $m['h1_last'] === 8 && $m['h1s'] === ['A'] && $m['sc_h'] === 0 && $m['sc_a'] === 1) && !($m['h2c'] === 1 && $m['h2_first_min'] < 7) && !($m['h1_first'] === 2 && $m['h1_last'] === 9 && $m['h1s'] === ['A','A'] && $m['sc_h'] === 0 && $m['sc_a'] === 2)
            )),
        ],
        [
            'id' => 'LG8',
            'label' => 'AWAY shortlist: Norway / Nigeria / Poland / Slovenia / Romania / Argentina / India / Belgium (20min, away lead HT, last gol 1H >=6, gol 1H>=2, bukan h1c=2 first=0, bukan AAHAA first=1 last=10, bukan AAAA 0-4 mnt 0-9, bukan AA 0-2 mnt 4-6, bukan all AWAY + HOME 0 + last<=8 + max_gap<=2)',
            'data' => array_values(array_filter($matches, fn($m) =>
                $m['league'] === '20min' && in_array(trim($m['away']), $lg8_teams, true) && $m['sc_a'] > $m['sc_h'] && $m['h1_last'] >= 6 && $m['h1c'] >= 2 && !($m['h1c'] === 2 && $m['h1_first'] === 0) && !($m['h1_first'] === 1 && $m['h1_last'] === 10 && $m['h1s'] === ['A','A','H','A','A']) && !($m['h1_first'] === 0 && $m['h1_last'] === 9 && $m['h1s'] === ['A','A','A','A'] && $m['sc_h'] === 0 && $m['sc_a'] === 4) && !($m['h1_first'] === 4 && $m['h1_last'] === 6 && $m['h1s'] === ['A','A'] && $m['sc_h'] === 0 && $m['sc_a'] === 2) && !($m['switches'] === 0 && $m['sc_h'] === 0 && $m['sc_a'] >= 2 && $m['h1_last'] <= 8 && $m['max_gap'] <= 2)
            )),
        ],
        [
            'id' => 'G24',
            'label' => 'Target gol 2H >4: 20min h1c=3 + span=8 + diff=1 + switches=1 + last HOME, atau 20min first=0 + span=8 + HT seri + last AWAY, atau 15min h1c=3 + span=4 + diff=1 + switches=2 + last AWAY, atau 20min AA mnt 3-5, bukan 15min AAA 0-3 mnt 2-7, bukan 15min AHA 1-2 mnt 2-6',
            ...
            'data' => array_values(array_filter($matches, fn($m) =>
                ...
                && !($m['league'] === '15min' && $m['h1_first'] === 2 && $m['h1_last'] === 6 && $m['h1s'] === ['A','H','A'] && $m['sc_h'] === 1 && $m['sc_a'] === 2)
            )),
        ],
    ];

...
// Deteksi apakah satu league-header sedang tertutup (collapsed) sehingga match-group
// tidak ter-render. Cek dari sinyal yang paling andal -> paling longgar, tanpa
// memicu false-positive yang malah menutup grup yang sebenarnya sudah terbuka.
function isLeagueCollapsed(header) {
    const ariaEl = header.matches('[aria-expanded]') ? header : header.querySelector('[aria-expanded]');
    if (ariaEl) {
        return ariaEl.getAttribute('aria-expanded') === 'false';
    }

    if (/collaps|closed/i.test(header.className || '')) {
        return true;
    }
    ...
    // Grup ada tapi disembunyikan via CSS -> tertutup. (display:none hanya terjadi
    // saat collapsed, jadi tidak akan keliru menutup grup yang terbuka.)
    try {

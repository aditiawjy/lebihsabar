...
// Rule per-MATCH: home + away + market + odd. Cocok bila pasangan tim sama
// (urutan home/away fleksibel karena odd O/U adalah level-match).
function normalizeWatchMatchRule(rule = {}) {
    const home = normalizeTeamName(rule.home || rule.homeTeam);
    const away = normalizeTeamName(rule.away || rule.awayTeam);
    if (!home || !away) return null;
    return {
        home,
        away,
        customOddThreshold: toThresholdNumber(rule.customOddThreshold ?? rule.threshold ?? rule.odd, DEFAULT_CUSTOM_WATCH_CONFIG.customOddThreshold),
        customOddSelection: normalizeWatchMarketSelection(rule.customOddSelection ?? rule.market ?? rule.selection, DEFAULT_CUSTOM_WATCH_CONFIG.customOddSelection)
    };
}

function normalizeWatchMatchRules(raw = []) {
    if (!Array.isArray(raw)) return [];
    const out = []; const seen = new Set();
    for (const r of raw) {
        const n = normalizeWatchMatchRule(r);
        if (!n) continue;
        const key = n.home + '|' + n.away;
...
function getMatchRuleByMatch(match, matchRules = []) {
    if (!Array.isArray(matchRules) || !matchRules.length) return null;
    const h = normalizeTeamName(match?.homeTeam);
    const a = normalizeTeamName(match?.awayTeam);
    if (!h || !a) return null;
    const inc = (x, y) => x && y && (x.includes(y) || y.includes(x));
    for (const rule of matchRules) {
        if ((inc(h, rule.home) && inc(a, rule.away)) || (inc(h, rule.away) && inc(a, rule.home))) {
            return rule;
        }
    }
    return null;
...
function normalizeWatchTeamRule(rule = {}, fallback = {}) {
    ...
    return {
        ...
        halftimeScores: normalizeHalftimeScores(rule.halftimeScores ?? rule.htScores ?? rule.halftimeScore),
        htOnly: Boolean(rule.htOnly)
    };
}

// Normalkan daftar skor HT ke bentuk kanonik "H-A" (tanpa spasi). Terima array
// atau string dipisah koma; buang yang tidak valid. Kosong = tidak ada syarat HT.
function normalizeHalftimeScores(raw) {
    let list = [];
    ...
    for (const item of list) {
        const m = String(item || '').replace(/\s+/g, '').match(/^(\d+)-(\d+)$/);
        if (m) {
...
async function getCustomWatchConfig() {
    ...
    return {
        ...
        matchRules: normalizeWatchMatchRules(stored.matchRules),
        customOddThreshold: fallbackThreshold,
        customOddSelection: fallbackSelection
    };
}
    ...
    for (const rule of customTeamRules) {
        ...
        if (matched) {
            return {
                ...
                halftimeScores: normalizeHalftimeScores(rule.halftimeScores),
                htOnly: Boolean(rule.htOnly)
            };
        }
    }
...
function getMatchWatchContext(match, customConfig = {}) {
    // Prioritas tertinggi: rule per-match (home+away+market+odd).
    const matchRule = getMatchRuleByMatch(match, customConfig.matchRules || []);
    if (matchRule) {
        return {
            isWatched: true,
            watchMatchRule: matchRule,
            appliedThreshold: toThresholdNumber(matchRule.customOddThreshold, TARGET_ODD_MIN),
            appliedSelection: normalizeWatchMarketSelection(matchRule.customOddSelection, TARGET_ODD_SELECTION),
            appliedBy: matchRule.home + ' vs ' + matchRule.away,
            htOnly: false
        };
    }
    ...
    return {
        ...
        htOnly: isWatched ? Boolean(matchTeamRule?.htOnly) : false
    };
}
...
async function setCustomWatchConfig(payload = {}) {
    ...
    const normalized = {
        ...
        matchRules: normalizeWatchMatchRules(payload.matchRules),
        customOddThreshold: fallbackThreshold,
...
function formatMatchMessage(match, targetOdd, watchContext = null) {
    ...
    const lines = [
        ...
        `👀 Watch: ${isWatchedTeam ? 'Ya' : 'Tidak'}${watchContext?.watchMatchRule ? ' (match rule)' : (watchContext?.appliedBy ? ' (' + escapeHtml(watchContext.appliedBy) + ')' : '')}`,

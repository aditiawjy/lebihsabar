...
function resetMatchTrackingState(key) {
	...
	ht1x2OddsByMatchKey.delete(key);
...
function parseScoreTuple(scoreStr) {
	const parts = String(scoreStr || "0-0")
		.split("-")
...
function shouldResetFixtureState(key, minute, scoreStr, nowMs) {
	...
	if (
		prevStatus.half === curStatus.half &&
		prevStatus.min >= 0 &&
		curStatus.min >= 0 &&
		curStatus.min + 2 < prevStatus.min
	) {
		return true;
	}
	...
	if (
		(last1HGoalMinByMatchKey.get(key) || -1) >= 7 &&
		curStatus.half === "1H" &&
		curStatus.min >= 0 &&
		curStatus.min <= 4
	) {
	...
	if (
		has2HGoalByMatchKey.get(key) &&
		(isHalftime || (curStatus.half === "2H" && curStatus.min <= 2))
	) {
		return true;
	}
...
function normalizeHtOddLabel(value) {
	return String(value || "")
		.toLowerCase()
		.replace(/\s+/g, "")
		.replace(/[.]/g, "");
...
function isHt1x2MarketName(marketName) {
	const normalized = String(marketName || "")
		.toLowerCase()
		.replace(/\s+/g, "")
		.replace(/[._-]/g, "");

	const hasHalfIndicator =
		normalized.includes("1h") ||
		normalized.includes("ht") ||
		normalized.includes("1sthalf") ||
		normalized.includes("firsthalf") ||
		normalized.includes("halftime");
	const hasThreeWayIndicator =
		normalized.includes("1x2") ||
		normalized.includes("winner") ||
		normalized.includes("result");

	return hasHalfIndicator && hasThreeWayIndicator;
}

function extractHt1x2Odds(match) {
	const result = { ht_w1: "", ht_x: "", ht_w2: "" };
	...
	for (const marketLine of odds) {
		...
		if (!isHt1x2MarketName(marketName)) continue;
		...
		for (const option of options) {
			...
			const label = normalizeHtOddLabel(option.slice(0, separatorIndex));
			const oddText = option.slice(separatorIndex + 1).trim();
			if (!oddText || oddText === "[LOCKED]") continue;

			if (!result.ht_w1 && ["1", "w1", "home", "h"].includes(label))
				result.ht_w1 = oddText;
			if (!result.ht_x && ["x", "draw", "d", "tie"].includes(label))
				result.ht_x = oddText;
			if (!result.ht_w2 && ["2", "w2", "away", "a"].includes(label))
				result.ht_w2 = oddText;
		}

		if (result.ht_w1 || result.ht_x || result.ht_w2) {
			return result;
		}
	}
...
function hasAnyHtOdds(odds) {
	return Boolean(odds && (odds.ht_w1 || odds.ht_x || odds.ht_w2));
}

function isHtOddsCaptureWindow(match) {
	const status = String(match?.status || "").trim();
	return /^H\.?Time$/i.test(status) || /^2H\s+0'$/i.test(status);
}

function buildHtOddsPayload(match) {
	if (!isHtOddsCaptureWindow(match)) {
		return {};
	}

	const key = createMatchKey(match);
	const current = extractHt1x2Odds(match);
	if (hasAnyHtOdds(current)) {
		ht1x2OddsByMatchKey.set(key, current);
		return current;
	}
...
function appendGoalState(
	...
	const prevParts = String(prevScore)
		.split("-")
		.map((value) => parseInt(value, 10) || 0);
	...
	if (half === "1H") {
		...
		if (!first1HGoalMinByMatchKey.has(key))
			first1HGoalMinByMatchKey.set(key, minuteValue);
...
function buildVisibleGoalBackfill(key, match, minute, scoreStr, timestamp) {
	...
	if (all1HGoalMinsByMatchKey.has(key) || all2HGoalMinsByMatchKey.has(key))
		return null;
...
async function trackGoalEvents(matches) {
	...
	for (const match of matches) {
		...
		const isTrackableLiveState =
			parsedMinute.half === "1H" ||
			parsedMinute.half === "2H" ||
			/^H\.?Time$/i.test(minute) ||
			/^Live$/i.test(minute);
		...
		if (
			...
			const kickoffBackfill = buildVisibleGoalBackfill(
				key,
				match,
				minute,
				scoreStr,
				kickoffTimeByMatchKey.get(key) || timestamp,
			);
			if (kickoffBackfill) {
		...
		if (
			(isHalftime || isSecondHalfStart(minute)) &&
			!shScoreByMatchKey.has(key)
		) {
		...
		if (prev === undefined) {
			const inferredGoal = buildVisibleGoalBackfill(
				key,
				match,
				minute,
				scoreStr,
	...
	for (const match of matches) {
		...
		if (!registeredMatchKeys.has(key)) continue;
		scoreSnapshots.push(
			buildScoreSnapshot(match, kickoffTimeByMatchKey.get(key) || timestamp),
		);
	}
	...
	for (const match of matches) {
		const key = createMatchKey(match);
		if (!registeredMatchKeys.has(key)) continue;
		const trackedEvents = buildTrackedGoalEventsForMatch(
			key,
			match,
			kickoffTimeByMatchKey.get(key) || timestamp,
		);
		const trackedFinal = trackedEvents.length
			? trackedEvents[trackedEvents.length - 1].score_after
			: null;
		if (
			trackedEvents.length &&
			trackedFinal ===
				`${String(match?.homeScore ?? "0").trim()}-${String(match?.awayScore ?? "0").trim()}`
		) {
...
function buildP727374State(match) {
	...
	if (
		...
		htScoreText &&
		(currentScore.home !== score.home || currentScore.away !== score.away)
	) {
		return null;
	}
	...
	const state = {
		...
		all_gaps_ge3:
			mins.length < 2 ||
			mins.every((min, idx) => idx === 0 || min - mins[idx - 1] >= 3),
		kickoff_hour: hasKickoffDate ? kickoffDate.getHours() : -1,
		kickoff_minute: hasKickoffDate ? kickoffDate.getMinutes() : -1,
		kickoff_dow: hasKickoffDate
			? ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][kickoffDate.getDay()]
			: "",
...
async function getP727374SignatureConfig() {
	...
	const response = await fetch(
		"http://localhost/lebihsabar/pattern-live-signatures.php",
		{ cache: "no-store" },
	);
	if (!response.ok)
		throw new Error(`pattern signatures HTTP ${response.status}`);
...
async function trackP727374GoalSignal(matches) {
	...
	try {
		...
		await setStatus({
			error: `P72 signatures failed: ${error.message || error}`,
		});
		return;
	}
...
async function trackP28Signal(matches) {
	...
	for (const match of matches) {
		...
		const isTargetTeam = ["croatia", "france"].some(

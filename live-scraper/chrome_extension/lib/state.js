...
var isLiveCycleRunning = false;
var currentTabId = null;
var lastAutoSentSignature = null;
var notifiedMatchKeys = new Set();
var wasAboveThresholdByMatchKey = new Map();
var oddHistoryByMatchKey = new Map();
var oddInsightByMatchKey = new Map();
...
let ht1x2OddsByMatchKey = new Map();
...
var lastTargetTabReloadAt = 0;

function getLiveStateRetentionMs() {
	return typeof MATCH_STATE_RETENTION_MS === "number"
		? MATCH_STATE_RETENTION_MS
		: 2 * 60 * 60 * 1000;
}
...
function pruneSetByKnownMatches(sourceSet) {
	return new Set(
		Array.from(sourceSet || []).filter((key) => registeredMatchKeys.has(key)),
	);
}

function pruneMilestoneSetByKnownMatches(sourceSet) {
	return new Set(
		Array.from(sourceSet || []).filter((key) => {
...
function pruneLongRunningMatchState(activeMatches = [], nowMs = Date.now()) {
	const activeKeys = new Set(
		(Array.isArray(activeMatches) ? activeMatches : []).map((match) =>
			createMatchKey(match),
		),
	);
	...
	if (registeredMatchKeys.size - staleKeys.length > maxKeys) {
		...
		Array.from(registeredMatchKeys)
			...
			.sort(
				(a, b) =>
					(lastSeenAtByMatchKey.get(a) || 0) -
					(lastSeenAtByMatchKey.get(b) || 0),
			)
...
function objectFromMapForActiveMatches(map, activeKeys) {
	return Object.fromEntries(
		Array.from(map.entries()).filter(([key]) => activeKeys.has(key)),
	);
}
...
function buildActiveMatchStatePayload(matches = []) {
	...
	return {
		activeKeys,
		allGoalMinutes: objectFromMapForActiveMatches(
			all1HGoalMinsByMatchKey,
			activeKeys,
		),
		allGoalScorers: objectFromMapForActiveMatches(
			all1HScorersByMatchKey,
			activeKeys,
		),
		all2HGoalMinutes: objectFromMapForActiveMatches(
			all2HGoalMinsByMatchKey,
			activeKeys,
		),
		all2HScorers: objectFromMapForActiveMatches(
			all2HScorersByMatchKey,
			activeKeys,
		),
		kickoffTimes: objectFromMapForActiveMatches(
			kickoffTimeByMatchKey,
			activeKeys,
		),
		patternSignals: objectFromMapForActiveMatches(
			p727374SignalsByMatchKey,
			activeKeys,
		),
...
async function updateLiveState(isRunning, extraState = {}) {
	...
	const state = await saveRuntimeState(extraState);
	...
	try {
		...
		await setStatus({
			error: `Alarm create failed: ${error?.message || error}`,
		});
	}
...
async function restoreRuntimeState() {
	...
	currentTabId = Number.isInteger(runtimeState.currentTabId)
		? runtimeState.currentTabId
		: null;
	...
	if (sessionData.ht1x2Odds) {
		ht1x2OddsByMatchKey = new Map(Object.entries(sessionData.ht1x2Odds));
	}
	if (sessionData.last1HGoalMins) {
		last1HGoalMinByMatchKey = new Map(
			Object.entries(sessionData.last1HGoalMins).map(([k, v]) => [
				k,
				Number(v),
			]),
		);
	...
	if (sessionData.first1HGoalMins) {
		first1HGoalMinByMatchKey = new Map(
			Object.entries(sessionData.first1HGoalMins).map(([k, v]) => [
				k,
				Number(v),
			]),
		);
	...
	if (sessionData.all1HGoalMins) {
		all1HGoalMinsByMatchKey = new Map(
			Object.entries(sessionData.all1HGoalMins).map(([k, v]) => [
				k,
				Array.isArray(v)
					? v.map((min) => Number(min)).filter((min) => Number.isFinite(min))
					: [],
			]),
		);
	}
	...
	if (sessionData.all2HGoalMins) {
		all2HGoalMinsByMatchKey = new Map(
			Object.entries(sessionData.all2HGoalMins).map(([k, v]) => [
				k,
				Array.isArray(v)
					? v.map((min) => Number(min)).filter((min) => Number.isFinite(min))
					: [],
	...
	if (sessionData.lastSeenAt) {
		lastSeenAtByMatchKey = new Map(
			Object.entries(sessionData.lastSeenAt).map(([k, v]) => [k, Number(v)]),
		);
	}
	...
	if (sessionData.p727374Signals) {
		p727374SignalsByMatchKey = new Map(
			Object.entries(sessionData.p727374Signals),
		);
	}
	...
	if (isLiveRunning) {
		try {
			...
			await setStatus({
				error: `Alarm restore failed: ${error?.message || error}`,
			});
		}
	}
}
...
async function persistMatchState(activeMatches = []) {
	...
	await Promise.all([
		chrome.storage.session.set({
			...
			ht1x2Odds: Object.fromEntries(ht1x2OddsByMatchKey),

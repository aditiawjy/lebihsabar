...
class TelegramNotifier:
    def __init__(self):
        ...
        # Batas entri agar memori tidak tumbuh tanpa batas pada server yang jalan lama
        self.max_history_entries = 300
        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code == 429:
                    # Hormati rate limit Telegram (retry_after dalam detik)
                    retry_after = (response.json() or {}).get("parameters", {}).get("retry_after", 3)
    ...
    def track_goal_minutes(self, match_data):
        ...
        if match_name not in self.match_score_history:
            # Buang entri tertua bila melebihi batas (dict Python menjaga urutan insert)
            while len(self.match_score_history) >= self.max_history_entries:
                oldest = next(iter(self.match_score_history))
                del self.match_score_history[oldest]
